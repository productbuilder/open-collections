export const COLLECTION_METADATA_DEFAULT_FIELDS = ['title', 'description', 'license', 'publisher', 'language'];
export const ITEM_OVERRIDE_FIELDS = ['description', 'license', 'attribution', 'language'];
...
export function resolveItemMetadata(item = {}, collection = {}) {
  ...
  for (const field of ITEM_OVERRIDE_FIELDS) {
    ...
    resolved.metadataResolution[field] = {
      ...
      overrideValue: explicitOverride.active ? explicitOverride.value : '',
      isInherited: !explicitOverride.active && Boolean(inheritedValue),
      source: explicitOverride.active ? explicitOverride.source : (inheritedValue ? 'collection' : (rawValue ? 'item' : 'empty')),
    };
  }
  ...
  return {
    ...
    overrideFields: ITEM_OVERRIDE_FIELDS.reduce((accumulator, field) => {
      ...
      accumulator[field] = {
        ...
        overrideValue: resolution.overrideActive ? resolution.overrideValue || '' : '',
...
export function clearItemOverride(item = {}, field) {
  const nextOverrides = item?.overrides && typeof item.overrides === 'object' ? { ...item.overrides } : {};
  delete nextOverrides[field];
  return nextOverrides;
}

export function getItemOverridePatch(editorState = {}, collection = {}, previousItem = {}) {
  const nextOverrides = previousItem?.overrides && typeof previousItem.overrides === 'object'
    ? { ...previousItem.overrides }
    : {};

  for (const field of ITEM_OVERRIDE_FIELDS) {
    if (editorState?.overrideStates?.[field]) {
      nextOverrides[field] = normalizeText(editorState?.overrides?.[field]);
    } else {
      delete nextOverrides[field];
    }
  }
  ...
  return {
    ...
    tags: Array.isArray(editorState?.tags) ? editorState.tags.map((tag) => normalizeText(tag)).filter(Boolean) : [],
...
export function getResolvedManifestItem(item = {}, collection = {}) {
  const resolvedItem = resolveItemMetadata(item, collection);
  const overrides = resolvedItem.overrides && typeof resolvedItem.overrides === 'object' ? resolvedItem.overrides : {};
  ...
  return {
    ...
    ...(Object.keys(overrides).length > 0 ? { overrides: { ...overrides } } : {}),
  };
}
