...
class OpenCollectionsBrowseShellElement extends HTMLElement {
	...
	syncListBrowseTypeControls() {
		const row = this.shadowRoot.querySelector(".browse-type-row");
		if (!row) {
			return;
		}
		const currentState = this.state.listBrowseModeState || {};
		const activeMode = toText(currentState.activeMode).toLowerCase() || "all";
		const disabledModes = currentState.disabledModes || {};
		row.querySelectorAll(".browse-type-chip[data-mode]").forEach((button) => {
			if (!(button instanceof HTMLButtonElement)) {
				return;
			}
			const mode = toText(button.getAttribute("data-mode")).toLowerCase();
			button.dataset.active = mode === activeMode ? "true" : "false";
			button.disabled = Boolean(disabledModes[mode]);
		});
	}
