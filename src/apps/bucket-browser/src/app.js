...
class PbWorkspaceBrowserElement extends HTMLElement {
	...
	connectedCallback() {
		...
		this.bootstrap().catch((error) => {
			this.setStatus(
				`Failed to load workspace scaffold: ${error.message}`,
				"warn",
			);
			this.renderApp();
		});
	...
	bindEvents() {
		...
		this.shadowRoot.addEventListener("path-change", (event) => {
			...
			this.state.expandedPaths = Array.from(
				new Set([
		...
		this.shadowRoot.addEventListener("view-mode-change", (event) => {
			this.state.currentViewMode =
				event.detail.mode === "list" ? "list" : "grid";
			this.renderApp();
		});
		...
		this.shadowRoot.addEventListener("asset-selection-toggle", (event) => {
			this.state.selectedAssetIds = toggleSelection(
		...
		this.shadowRoot.addEventListener("bulk-action", async (event) => {
			const result = await this.service.performBulkAction(
	...
	async loadWorkspace(workspaceId) {
		...
		const { workspace, treeNodes, assets } =
			await this.service.loadWorkspace(workspaceId);
		this.state.activeWorkspaceId = workspace?.id || null;
		this.state.treeNodes = treeNodes;
		this.state.assets = assets;
		this.state.activePath =
			treeNodes.find((node) => node.path === "/")?.path ||
			treeNodes[0]?.path ||
			"/";
		this.state.expandedPaths = createExpandedPaths(
			treeNodes,
			this.state.activePath,
		);
		this.state.selectedAssetIds = [];
		...
		this.ensureFocusedVisibleAsset();
		this.setStatus(
			`Loaded ${workspace?.name || "workspace"} scaffold from workspace service.`,
			"ok",
		);
		this.renderApp();
	}
	...
	ensureFocusedVisibleAsset() {
		...
		const visibleIds = new Set(
			viewModel.visibleAssets.map((asset) => asset.id),
		);
		if (!visibleIds.has(this.state.focusedAssetId)) {
			this.state.focusedAssetId = viewModel.visibleAssets[0]?.id || null;
		}
		if (
			this.state.previewAssetId &&
			!visibleIds.has(this.state.previewAssetId)
		) {
			this.state.previewAssetId = null;
		}
		this.state.selectedAssetIds = this.state.selectedAssetIds.filter(
			(assetId) => visibleIds.has(assetId),
		);
	}
	...
	renderApp() {
		...
		this.dom.layout.setAttribute(
			"mobile-tree-open",
			this.state.mobileTreeOpen ? "true" : "false",
		);
		this.dom.layout.setAttribute(
			"mobile-details-open",
			this.state.mobileDetailsOpen ? "true" : "false",
		);
	}
}
