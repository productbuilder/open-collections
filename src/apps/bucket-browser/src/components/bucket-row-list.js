...
class PbBucketRowListElement extends HTMLElement {
  ...
  bindEvents() {
    this.shadowRoot.querySelectorAll('tbody tr[data-focus-id]').forEach((row) => {
      row.addEventListener('click', (event) => {
        if (event.target.closest('input[type="checkbox"], button')) {
          return;
        }
        this.dispatch('asset-focus', { assetId: row.dataset.focusId });
      });
    });
  ...
  render() {
    const rows = this.model.assets.map((asset) => {
