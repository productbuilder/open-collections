...
class PbWorkspaceToolbarElement extends HTMLElement {
  ...
  bindEvents() {
    this.shadowRoot.getElementById('workspaceSelect')?.addEventListener('change', (event) => {
      this.dispatch('workspace-change', { workspaceId: event.target.value });
    });
    this.shadowRoot.getElementById('viewModeSelect')?.addEventListener('change', (event) => {
      this.dispatch('view-mode-change', { mode: event.target.value });
    });
    this.shadowRoot.getElementById('searchInput')?.addEventListener('input', (event) => {
      this.dispatch('search-change', { query: event.target.value });
    });
    this.shadowRoot.getElementById('availabilityFilter')?.addEventListener('change', (event) => {
      this.dispatch('filters-change', { availability: event.target.value });
    });
    this.shadowRoot.getElementById('kindFilter')?.addEventListener('change', (event) => {
      this.dispatch('filters-change', { kind: event.target.value });
    });
    this.shadowRoot.getElementById('toggleTreeBtn')?.addEventListener('click', () => {
      this.dispatch('toggle-mobile-tree');
    });
    this.shadowRoot.getElementById('toggleDetailsBtn')?.addEventListener('click', () => {
  ...
  render() {
    const workspaceOptions = this.model.workspaces.map((workspace) => `
