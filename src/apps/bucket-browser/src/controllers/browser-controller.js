...
export function createExpandedPaths(treeNodes, activePath = "/") {
	const availablePaths = new Set(
		treeNodes.map((node) => normalizePath(node.path)),
	);
	return getAncestorPaths(activePath).filter((path) =>
...
export function ensureExpandedPath(expandedPaths, treeNodes, path) {
	...
	return getAncestorPaths(path).filter(
		(ancestorPath) =>
			availablePaths.has(ancestorPath) ||
			expandedPaths.includes(ancestorPath),
	);
}
...
export function buildTreeHierarchy(treeNodes = []) {
	...
	const sortedNodes = [...nodesByPath.values()].sort((left, right) =>
	...
	sortedNodes.forEach((node) => {
		node.children.sort((left, right) =>
			comparePaths(left.label || left.path, right.label || right.path),
		);
	});
...
export function getVisibleAssets(state) {
	...
	return state.assets.filter((asset) => {
		...
		const haystack =
			`${asset.name} ${asset.summary} ${asset.kind} ${asset.syncState}`.toLowerCase();
...
export function resolveFocusedAsset(state) {
	return (
		state.assets.find((asset) => asset.id === state.focusedAssetId) || null
	);
}

export function resolvePreviewAsset(state) {
	return (
		state.assets.find((asset) => asset.id === state.previewAssetId) || null
	);
}
...
export function ensureFocusState(state) {
	...
	state.selectedAssetIds = state.selectedAssetIds.filter((assetId) =>
