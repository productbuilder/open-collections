...
import { renderCreateNewFolderIcon } from "../../../shared/components/icons.js";
...
class OpenCollectionsConnectorElement extends HTMLElement {
	...
	cacheDom() {
		this.dom = {
			...
			connectionsAddView: this.shadow.getElementById("connectionsAddView"),
			...
			connectionDetailPanel:
				this.shadow.getElementById("connectionDetailPanel"),
			connectionDetailSaveBtn:
				this.shadow.getElementById("connectionDetailSaveBtn"),
			connectionDetailSaveStatus: this.shadow.getElementById(
				"connectionDetailSaveStatus",
			),
			connectionDetailBackBtn:
				this.shadow.getElementById("connectionDetailBackBtn"),
	...
	bindEvents() {
		...
		this.dom.connectionDetailPanel?.addEventListener(
			...
			(event) => {
				...
				if (!sourceId || sourceId !== this.state.activeSourceId) return;
		...
		this.dom.connectionDetailPanel?.addEventListener(
			...
			async (event) => {
				const sourceId = event.detail?.sourceId || "";
				if (sourceId) await this.refreshSource(sourceId);
			},
		);
		this.dom.connectionDetailPanel?.addEventListener(
			"repair-connection",
				...
				if (mode === "folder") {
					...
					if (didPick) {
						await this.refreshSource(sourceId, {
							configOverrides: {
								localDirectoryHandle: this.selectedLocalDirectoryHandle,
							},
						});
					}
					return;
		...
		this.dom.connectionDetailPanel?.addEventListener("remove-connection", (event) => {
			const sourceId = event.detail?.sourceId || "";
			if (sourceId) this.removeSource(sourceId);
		});
		this.dom.connectionDetailPanel?.addEventListener(
			...
			(event) => {
				...
				if (sourceId) this.toggleExampleConnection(sourceId, enabled);
			},
		);
		...
		this.dom.addConnectionPanel?.addEventListener("select-provider", (event) => {
			...
			if (providerId) this.setSelectedProvider(providerId);
		});
		...
		this.dom.addConnectionPanel?.addEventListener(
			...
			async () => {
				...
				this.setSelectedProvider("local");
				const didPick = await this.pickLocalFolder();
				if (didPick) await this.connectCurrentProvider();
			},
		);
	}
	...
	openCredentialRepairView(sourceId) {
		...
		if (!source || !["github", "s3"].includes(source.providerId)) return;
	...
	handleSessionConnectionSourcesChanged(sources = []) {
		...
		if (incomingById.size === 0) return;
		const existingById = new Map(this.state.sources.map((entry) => [entry.id, entry]));
		this.state.sources = Array.from(incomingById.values()).map((source) => ({
	...
	getSourceTitle(source) {
		if (!source) return "";
		return source.displayLabel || source.label || source.providerLabel || "Connection";
	}
	...
	syncConnectionDetailState(source) {
		...
		if (this.state.connectionDetailStateSourceId === source.id) return;
	...
	renderConnectionDetailSaveState() {
		...
		const state = hasSource ? this.state.connectionDetailSaveState : "idle";
	...
	async ensureStarterExampleConnection() {
		...
		if (starterExampleSource) return;
	...
	async removeSource(sourceId) {
		...
		this.persistSources();
		this.renderConnectionsListPanel();
		if (removedWhileInspecting) this.showConnectionsListView();
	...
	toggleExampleConnection(sourceId, enabled) {
		const source = this.getSourceById(sourceId);
		if (!source || source.providerId !== "example") return;
...
if (!customElements.get("open-collections-connector")) {
	customElements.define(
		"open-collections-connector",
		OpenCollectionsConnectorElement,
	);
}

export { OpenCollectionsConnectorElement };
