...
class OpenCollectionsAssetViewerElement extends HTMLElement {
	...
	dispatchCloseEvent() {
		this.dispatchEvent(
			new CustomEvent("close-viewer", { bubbles: true, composed: true }),
		);
	}

	setItem(item, formatSourceBadgeFn = null) {
		this.item = item || null;
		this.formatSourceBadgeFn =
			typeof formatSourceBadgeFn === "function"
				? formatSourceBadgeFn
				: null;
		this.renderItem();
	}
	...
	renderItem() {
		...
		if (!this.item) {
			...
			placeholder.textContent =
				"Select an item to view its media preview.";
		...
		licenseBadge.textContent = hasLicense
...
if (!customElements.get("open-collections-asset-viewer")) {
	customElements.define(
		"open-collections-asset-viewer",
		OpenCollectionsAssetViewerElement,
	);
}
