...
class OpenCollectionsMetadataElement extends HTMLElement {
	...
	bindEvents() {
		...
		this.shadowRoot
			.getElementById("headerSaveBtn")
			?.addEventListener("click", () => {
				if (this.model.mode === "collection") {
					this.dispatch("save-collection", {
						patch: this.getCollectionPatch(),
					});
					return;
				}
		...
		this.shadowRoot
			.getElementById("headerDeleteBtn")
			?.addEventListener("click", () => {
				if (
		...
		this.shadowRoot.addEventListener("click", (event) => {
			const button =
				event.target instanceof Element
					? event.target.closest("[data-override-toggle]")
					: null;
			if (!button) {
				return;
			}
	...
	setMobileOpen(open) {
		...
		if (panel) {
			panel.classList.toggle(
				"is-mobile-editor-open",
				this.model.mobileOpen,
			);
		}
	}
	...
	setPresentation(presentation = "inspector") {
		this.model.presentation =
			presentation === "embedded" ? "embedded" : "inspector";
		const panel = this.shadowRoot?.querySelector(".editor-panel");
		if (panel) {
	...
	getCollectionPatch() {
		return {
			title:
				this.shadowRoot
					.getElementById("collectionEditorTitle")
					?.value.trim() || "",
			description:
				this.shadowRoot
					.getElementById("collectionEditorDescription")
					?.value.trim() || "",
			license:
				this.shadowRoot
					.getElementById("collectionEditorLicense")
					?.value.trim() || "",
			publisher:
				this.shadowRoot
					.getElementById("collectionEditorPublisher")
					?.value.trim() || "",
			language:
				this.shadowRoot
					.getElementById("collectionEditorLanguage")
					?.value.trim() || "",
		};
	}
	...
	getItemPatch() {
		...
		for (const [field, config] of Object.entries(
			ITEM_OVERRIDE_FIELD_CONFIG,
		)) {
			const enabled = this.isOverrideEnabled(field);
			overrideStates[field] = enabled;
			overrides[field] =
				this.shadowRoot.getElementById(config.inputId)?.value.trim() ||
				"";
		}

		return {
			title:
				this.shadowRoot.getElementById("itemTitle")?.value.trim() || "",
			creator:
				this.shadowRoot.getElementById("itemCreator")?.value.trim() ||
				"",
			date:
				this.shadowRoot.getElementById("itemDate")?.value.trim() || "",
			location:
				this.shadowRoot.getElementById("itemLocation")?.value.trim() ||
				"",
			source:
				this.shadowRoot.getElementById("itemSource")?.value.trim() ||
				"",
			tags: this.tagsToArray(
				this.shadowRoot.getElementById("itemTags")?.value || "",
			),
			include: Boolean(
				this.shadowRoot.getElementById("itemInclude")?.checked,
			),
			mediaType:
				this.shadowRoot.getElementById("itemType")?.value.trim() || "",
			overrides,
			overrideStates,
		};
	}
	...
	setOverrideEnabled(field, enabled) {
		...
		if (button) {
			button.textContent = nextEnabled
	...
	applyOverrideView(field, state = {}) {
		...
		if (input) {
			input.value = state.overrideActive
				? state.overrideValue || ""
				: state.resolvedValue || "";
		}
		if (preview) {
	...
	applyView() {
		...
		const collectionForm = this.shadowRoot?.getElementById(
		...
		const headerDeleteBtn =
			this.shadowRoot?.getElementById("headerDeleteBtn");
		...
		if (mode === "collection") {
			...
			this.shadowRoot.getElementById("collectionEditorTitle").value =
				selected.title || "";
			this.shadowRoot.getElementById(
				"collectionEditorDescription",
			).value = selected.description || "";
			this.shadowRoot.getElementById("collectionEditorLicense").value =
				selected.license || "";
			this.shadowRoot.getElementById("collectionEditorPublisher").value =
				selected.publisher || "";
			this.shadowRoot.getElementById("collectionEditorLanguage").value =
				selected.language || "";
			return;
		}
		...
		if (mode === "item") {
			...
			this.shadowRoot.getElementById("itemTitle").value =
				selected.itemSpecific?.title || selected.title || "";
			this.shadowRoot.getElementById("itemType").value =
				selected.itemSpecific?.mediaType || selected.media?.type || "";
			this.shadowRoot.getElementById("itemFilePath").value = filePath;
			this.shadowRoot.getElementById("itemCreator").value =
				selected.itemSpecific?.creator || selected.creator || "";
			this.shadowRoot.getElementById("itemDate").value =
				selected.itemSpecific?.date || selected.date || "";
			this.shadowRoot.getElementById("itemLocation").value =
				selected.itemSpecific?.location || selected.location || "";
			this.shadowRoot.getElementById("itemSource").value =
				selected.itemSpecific?.source || selected.source || "";
			this.shadowRoot.getElementById("itemTags").value = Array.isArray(
				selected.itemSpecific?.tags,
			)
				? selected.itemSpecific.tags.join(", ")
				: "";
			this.shadowRoot.getElementById("itemInclude").checked =
				selected.itemSpecific?.include !== false;

			for (const field of Object.keys(ITEM_OVERRIDE_FIELD_CONFIG)) {
				this.applyOverrideView(
					field,
					selected.overrideFields?.[field] || {},
				);
			}
			return;
		}
...
if (!customElements.get("open-collections-metadata")) {
	customElements.define(
		"open-collections-metadata",
		OpenCollectionsMetadataElement,
	);
}
