...
class OpenCollectionsSourceManagerElement extends HTMLElement {
	...
	bindEvents() {
		this.shadowRoot
			.getElementById("addExampleHostBtn")
		...
		this.shadowRoot
			.getElementById("addLocalFolderHostBtn")
			?.addEventListener("click", () => {
				this.model.flowMode = "add";
				this.model.repairProviderId = "";
				this.model.addHostLevel = "root";
		...
		this.shadowRoot
			.getElementById("addRemoteHostBtn")
			?.addEventListener("click", () => {
				this.model.flowMode = "add";
				this.model.repairProviderId = "";
	...
	renderSourcesList() {
		...
		for (const source of this.model.sources) {
			...
			detail.textContent =
				source.detailLabel ||
				`${source.providerLabel} | ${source.itemCount || 0} items`;
			...
			status.textContent =
				hostState.detail || source.status || "Connected";

			const meta = document.createElement("div");
			meta.className = "source-card-meta";
			const statusPill = document.createElement("span");
			statusPill.className =
				`pill ${hostState.tone === "warn" ? "is-warn" : hostState.tone === "ok" ? "is-ok" : ""}`.trim();
			statusPill.textContent = hostState.label;
			const activePill = document.createElement("span");
			activePill.className = `pill ${isActive ? "is-ok" : ""}`.trim();
			activePill.textContent = isActive
				? "Active connection"
				: "Inactive";
			const publishPill = document.createElement("span");
			publishPill.className =
				`pill ${source.capabilities?.canPublish ? "is-ok" : "is-muted"}`.trim();
			publishPill.textContent = source.capabilities?.canPublish
				? "Connected"
				: "Read-only";
...
if (!customElements.get("open-collections-source-manager")) {
	customElements.define(
		"open-collections-source-manager",
		OpenCollectionsSourceManagerElement,
	);
}
