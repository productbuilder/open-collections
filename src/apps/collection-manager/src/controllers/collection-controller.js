...
export function activeCollectionRootPath(app) {
	...
	const activeSource =
		app.state.activeSourceFilter !== "all"
			...
			: null;
	if (activeSource) {
		const sourceCollection = (activeSource.collections || []).find(
			(entry) => entry.id === collectionId,
		);
		if (sourceCollection?.rootPath) {
			return app.normalizeCollectionRootPath(
				sourceCollection.rootPath,
				collectionId,
			);
		}
	}
	const localCollection = app.state.localDraftCollections.find(
		(entry) => entry.id === collectionId,
	);
	if (localCollection?.rootPath) {
		return app.normalizeCollectionRootPath(
			localCollection.rootPath,
			collectionId,
		);
	}
...
export function ensureCollectionForSource(app, source) {
	...
	if (source.providerId === "local") {
		if (
			preferred &&
			preferred !== "all" &&
			existing.some((entry) => entry.id === preferred)
		) {
		...
		if (
			...
			existing.some((entry) => entry.id === source.selectedCollectionId)
		) {
	...
	if (preferred && preferred !== "all") {
		if (!existing.some((entry) => entry.id === preferred)) {
			const localEntry = app.state.localDraftCollections.find(
...
export function collectionLabelFor(source, collectionId) {
	const found = (source.collections || []).find(
		(entry) => entry.id === collectionId,
	);
...
export function refreshSourceCollectionsAndCounts(app, sourceId) {
	...
	source.collections = nextCollections.map((collection) => {
		const existing = (source.collections || []).find(
	...
	if (
		!source.collections.some(
			(entry) => entry.id === source.selectedCollectionId,
		)
	) {
...
export function renderCollectionFilter(app) {
	...
	app.dom.collectionBrowser.setCollectionOptions(
		options,
...
export function currentCollectionMeta(app) {
	const collectionId =
		app.dom.collectionId.value.trim() ||
		MANAGER_CONFIG.defaultCollectionMeta.id;
	return {
		id: collectionId,
		title:
			app.dom.collectionTitle.value.trim() ||
			MANAGER_CONFIG.defaultCollectionMeta.title,
		description:
			app.dom.collectionDescription.value.trim() ||
		...
		rootPath:
			app.activeCollectionRootPath() ||
			app.normalizeCollectionRootPath(`${collectionId}/`, collectionId),
	};
}
