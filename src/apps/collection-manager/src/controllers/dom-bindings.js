...
export function cacheDomElements(root) {
	return {
		...
		newCollectionDescription: root.getElementById(
...
export function initializeDomDefaults(app) {
	...
	app.dom.collectionDescription.value =
		MANAGER_CONFIG.defaultCollectionMeta.description;
...
export function bindDomEvents(app) {
	...
	app.dom.managerHeader?.addEventListener("open-header-menu", () =>
	...
	app.dom.connectionsListPanel.addEventListener(
		"select-connection",
		(event) => {
	...
	app.dom.addConnectionPanel.addEventListener("open-storage-options", () =>
		app.openDialog(app.dom.storageOptionsDialog),
	);
	...
	const bindBrowserSurface = (target) => {
		...
		target.addEventListener("view-mode-change", (event) => {
			app.setBrowserViewMode(
				event.detail?.level || "collections",
				event.detail?.mode || "cards",
			);
		});
		...
		target.addEventListener("attach-media-url", async (event) => {
			...
			const current = app.state.assets.find(
				(item) => item.workspaceId === itemId,
			);
			const initialValue =
				current?.media?.mode === "referenced"
					? String(current?.media?.url || "").trim()
					: "";
	...
	const bindMetadataSurface = (target) => {
		...
		target.addEventListener("save-item", async (event) => {
			...
			const rawEditorState =
				event.detail?.patch ||
				(typeof target.getItemPatch === "function"
					? target.getItemPatch()
					: null);
			const patch = app.buildItemPatchFromEditor(
				rawEditorState,
				selected,
			);
			await app.updateItem(selected.workspaceId, patch, {
				explicitSave: true,
			});
		});
		target.addEventListener("save-collection", async (event) => {
			const patch =
				event.detail?.patch ||
				(typeof target.getCollectionPatch === "function"
					? target.getCollectionPatch()
					: null);
	...
	app.dom.metadataEditor.addEventListener("close-editor", () =>
		app.closeMobileDetail(),
	);
	app.dom.mobileFlow?.addEventListener("back-to-browse", () =>
		app.closeMobileDetail(),
	);
	...
	app.dom.newCollectionTitle.addEventListener("input", () => {
		...
		if (!currentSlug) {
			app.dom.newCollectionSlug.value = app.slugifySegment(
				app.dom.newCollectionTitle.value.trim(),
				"new-collection",
			);
		}
	});
