...
export function currentWorkspaceSnapshot(app) {
	return {
		...
		selectedCollectionIds: Array.isArray(app.state.selectedCollectionIds)
			? [...app.state.selectedCollectionIds]
			: [],
		selectedItemId: app.state.selectedItemId || null,
		selectedItemIds: Array.isArray(app.state.selectedItemIds)
...
export function buildLocalDraftPayload(app) {
	return {
		...
		sources: app.state.sources.map((source) =>
...
export function applyWorkspaceSnapshot(app, snapshot = {}) {
	...
	if (
		snapshot.collectionMeta &&
		typeof snapshot.collectionMeta === "object"
	) {
		app.setCollectionMetaFields({
			id: snapshot.collectionMeta.id || app.dom.collectionId.value,
			title:
				snapshot.collectionMeta.title || app.dom.collectionTitle.value,
			description:
				snapshot.collectionMeta.description ||
				app.dom.collectionDescription.value,
			license:
				snapshot.collectionMeta.license ||
				app.dom.collectionLicense.value,
			publisher:
				snapshot.collectionMeta.publisher ||
				app.dom.collectionPublisher.value,
			language:
				snapshot.collectionMeta.language ||
	...
	if (
		...
		(snapshot.selectedSourceId === "all" ||
			app.state.sources.some(
				(entry) => entry.id === snapshot.selectedSourceId,
			))
	) {
		...
		const visibleSelection = snapshot.selectedCollectionIds.filter(
			(collectionId) =>
				...
				app
					...
					.some((entry) => entry.id === collectionId),
		);
	...
	if (
		...
		app.state.assets.some(
			(entry) => entry.workspaceId === snapshot.selectedItemId,
		)
	) {
...
export function applyLocalDraftPayload(app, payload) {
	...
	if (payload.collectionMeta && typeof payload.collectionMeta === "object") {
		app.setCollectionMetaFields({
			...
			title:
				payload.collectionMeta.title || app.dom.collectionTitle.value,
			description:
				payload.collectionMeta.description ||
				app.dom.collectionDescription.value,
			license:
				payload.collectionMeta.license ||
				app.dom.collectionLicense.value,
			publisher:
				payload.collectionMeta.publisher ||
				app.dom.collectionPublisher.value,
			language:
				payload.collectionMeta.language ||
				app.dom.collectionLanguage.value,
		});
	}
	...
	if (payload.manifest && typeof payload.manifest === "object") {
		...
		app.dom.manifestPreview.textContent = JSON.stringify(
			payload.manifest,
			null,
			2,
		);
	...
	if (
		...
		app.state.assets.some(
			(entry) => entry.workspaceId === payload.selectedItemId,
		)
	) {
...
export async function restoreRememberedSources(app) {
	...
	for (const entry of Array.isArray(remembered) ? remembered : []) {
		...
		const source = usingOpfsRemembered
			? {
					...
					label:
						...
						entry.displayLabel ||
						"Source",
					config: app.sanitizeSourceConfig(
						entry.providerId,
						entry.config || {},
					),
					capabilities:
						entry.capabilities ||
						app.providerFactories[
							entry.providerId
						]?.getCapabilities?.() ||
						{},
		...
		if (source.providerId === "local") {
			const rawHandle =
				...
				typeof entry.config.localDirectoryHandle === "object"
					? entry.config.localDirectoryHandle
					: null;
			const configuredPath = String(entry.config?.path || "").trim();
			const fallbackPath = String(
				entry.config?.localDirectoryPath ||
					(isFilesystemLikePath(configuredPath)
						? configuredPath
						: ""),
			).trim();
			const fallbackName = String(
				entry.config?.localDirectoryName || "",
			).trim();
			...
			if (revived?.kind === "directory") {
				source.config = {
					...
					localDirectoryName:
						...
						String(revived.name || fallbackName).trim(),
					path:
						source.config?.path ||
						String(revived.path || fallbackPath).trim(),
				};
		...
		if (source.providerId === "github") {
			logCredentialRestore("restore:load-secret:start", {
				...
				summary: summarizeCredentialConfig(
					source.providerId,
					source.config || {},
				),
			});
			const configWithToken = await app.credentialStore.loadSourceSecret(
				source,
				source.config || {},
			);
			source.config = configWithToken;
			source.authMode = (configWithToken.token || "").trim()
				? "token"
				: "public";
			...
			const configWithSecret = await app.credentialStore.loadSourceSecret(
				source,
				source.config || {},
			);
			source.config = configWithSecret;
			source.authMode = (configWithSecret.accessKey || "").trim()
				? "access-key"
				: "public";
			source.needsCredentials = !(
				(configWithSecret.accessKey || "").trim() &&
				(configWithSecret.secretKey || "").trim()
			);
			logCredentialRestore("restore:load-secret:done", {
				sourceId: source.id,
				identity,
	...
	try {
		const stored = JSON.parse(
			readLocalStorageString(WORKSPACE_SELECTION_STORAGE_KEY, "{}"),
		);
	...
	app.state.activeSourceFilter = isWebLikeRuntime
		? "all"
		: app.state.sources.some(
					(source) => source.id === desiredActiveSourceId,
			  )
