...
export function buildInitialCollectionManifest(manager, meta) {
	const rootPath = manager.normalizeCollectionRootPath(
		meta.rootPath || `${meta.id}/`,
		meta.id,
	);
	return {
		id: meta.id,
...
export async function createNewCollectionDraft(manager) {
	...
	if (!title) {
		manager.setStatus(
			"Enter a collection title to create a draft.",
			"warn",
		);
		return;
	}
	...
	if (
		...
		(!activeSource.provider || !activeSource.capabilities?.canSaveMetadata)
	) {
		manager.setStatus(
			"Reconnect the selected local host with write access before creating a collection.",
			"warn",
		);
		return;
	}
	...
	if (
		...
		try {
			...
			const nextMeta = {
				...
				rootPath: manager.normalizeCollectionRootPath(
					createdCollection?.rootPath || meta.rootPath,
					nextCollectionId,
				),
			};

			manager.setCollectionMetaFields(nextMeta);
			manager.state.manifest = buildInitialCollectionManifest(
				manager,
				nextMeta,
			);
			manager.dom.manifestPreview.textContent = JSON.stringify(
				manager.state.manifest,
				null,
				2,
			);
			...
			const exists = (activeSource.collections || []).some(
				(entry) => entry.id === nextCollectionId,
			);
			activeSource.collections = exists
				? (activeSource.collections || []).map((entry) =>
						entry.id === nextCollectionId
							? { ...entry, ...nextCollectionEntry }
							: entry,
					)
			...
			if (
				!manager.state.localDraftCollections.some(
					(entry) => entry.id === nextCollectionId,
				)
			) {
			...
			manager.setStatus(
				`Created collection ${nextCollectionId} in local host ${activeSource.displayLabel || activeSource.label}.`,
				"ok",
			);
			...
			manager.setStatus(
				`Local collection creation failed: ${error.message}`,
				"warn",
			);
			return;
		}
	...
	manager.state.selectedItemId = null;
	if (
		!manager.state.localDraftCollections.some((entry) => entry.id === slug)
	) {
		manager.state.localDraftCollections = [
			...manager.state.localDraftCollections,
			{
	...
	if (manager.state.activeSourceFilter !== "all") {
		...
		if (source) {
			const exists = (source.collections || []).some(
				(entry) => entry.id === slug,
			);
			if (!exists) {
				source.collections = [
...
export function openCollectionView(manager, collectionId) {
	...
	manager.state.managerBrowseMode = "items";
	manager.state.selectedItemId = null;
...
export function leaveCollectionView(manager) {
	manager.state.currentLevel = "collections";
	manager.state.managerBrowseMode = "collections";
	manager.state.openedCollectionId = null;
	manager.state.selectedItemId = null;
	manager.state.selectedItemIds = [];
	manager.syncMetadataModeFromState();
...
export async function saveSelectedCollectionMetadata(manager, patch = null) {
	...
	if (
		...
		manager.setStatus(
			"Reconnect the selected local host with write access before saving collection metadata.",
			"warn",
		);
		return;
	}
	...
	if (
		...
		try {
			const result = await activeSource.provider.saveCollectionMetadata(
				selected.id,
				nextPatch,
			);
			const updatedCollection = result?.collection || {};
			const normalizedRootPath = manager.normalizeCollectionRootPath(
				updatedCollection.rootPath ||
					selected.rootPath ||
					`${selected.id}/`,
				selected.id,
			);

			activeSource.collections = (activeSource.collections || []).map(
				(entry) =>
			...
			manager.state.localDraftCollections =
				manager.state.localDraftCollections.map((entry) =>
					entry.id === selected.id
						? {
								...entry,
								...nextPatch,
			...
			manager.state.assets = manager.state.assets.map((item) =>
				item.sourceId === activeSource.id &&
				item.collectionId === selected.id
					? {
							...item,
							collectionLabel:
			...
			manager.renderEditor();
			manager.setStatus(
				`Saved collection metadata for ${selected.id}.`,
				"ok",
			);
			manager.markSavedToSource();
			return;
			...
			manager.setStatus(
				`Saving collection metadata failed: ${error.message}`,
				"warn",
			);
			return;
		}
	...
	if (manager.state.activeSourceFilter !== "all") {
		...
		if (source?.collections) {
			source.collections = source.collections.map((entry) =>
				entry.id === selected.id ? { ...entry, ...nextPatch } : entry,
			);
		}
	}
	manager.state.localDraftCollections =
