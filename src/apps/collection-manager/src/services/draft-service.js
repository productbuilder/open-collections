export async function persistSourcesToOpfs(manager, payload) {
	...
	for (const source of payload) {
		await manager.opfsStorage.writeJsonFile(
			manager.sourceFilePath(source.id),
			source,
		);
	}
}
...
export async function initializeLocalDraftState(manager) {
	try {
		manager.state.opfsAvailable =
			await manager.opfsStorage.isOpfsAvailable();
	} catch (error) {
	...
	if (workspace) {
		...
		await manager.restoreLocalDraft({
			silent: true,
			preferredCollectionId: workspace.draftCollectionId || "",
		});
		if (manager.state.lastLocalSaveAt) {
			manager.setLocalDraftStatus(
				`Local draft ready (last saved ${manager.state.lastLocalSaveAt}).`,
				"ok",
			);
		}
	}
...
export async function saveLocalDraft(manager) {
	...
	try {
		...
		await manager.opfsStorage.writeJsonFile(
			manager.draftFilePath(collectionId),
			payload,
		);
		...
		await persistSourcesToOpfs(
			manager,
			manager.state.sources.map((source) =>
				manager.toPersistedSource(source),
			),
		);
		manager.setLocalDraftStatus(
			`Saved local draft at ${payload.savedAt}.`,
			"ok",
		);
...
export async function restoreLocalDraft(manager, options = {}) {
	...
	try {
		const payload = await manager.opfsStorage.readJsonFile(
			manager.draftFilePath(collectionId),
		);
		if (!payload) {
			if (!options.silent) {
				manager.setLocalDraftStatus(
					`No local draft found for ${collectionId}.`,
					"warn",
				);
			}
			return;
		...
		manager.state.lastLocalSaveAt = payload.savedAt || "";
		if (!options.silent) {
			const restoredSuffix = payload.savedAt
				? ` from ${payload.savedAt}`
				: "";
			manager.setLocalDraftStatus(
		...
		manager.setLocalDraftStatus(
			`Local draft restore failed: ${error.message}`,
			"warn",
		);
		if (!options.silent) {
			manager.setStatus(
...
export async function discardLocalDraft(manager) {
	...
	const collectionId = manager.draftCollectionId();
	try {
		await manager.opfsStorage.deleteFile(
			manager.draftFilePath(collectionId),
		);
		const workspace =
			(await manager.opfsStorage.readJsonFile("workspace.json")) || {};
		...
		manager.setLocalDraftStatus(
			`Discarded local draft ${collectionId}.`,
			"ok",
		);
		...
		manager.setLocalDraftStatus(
			`Discard draft failed: ${error.message}`,
			"warn",
		);
