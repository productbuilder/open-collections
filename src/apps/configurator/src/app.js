...
} from "./workspace/workspace-persistence.js";
...
const DEFAULT_CARD_SECTIONS = new Set([
	"products-collections",
	"materials-collections",
	"products-entries:blocks",
]);
...
function validateDataRoot(workspace, data) {
	...
	if (!isPlainObject(data)) {
		result.overviewWarnings.push(
			`${workspace} source root must be an object.`,
		);
		return result;
	}
	...
	if (workspace === "general") {
		if (!isPlainObject(data.info) && !isPlainObject(data.settings)) {
			result.overviewWarnings.push(
				"Organization info/settings are missing.",
			);
		}
	}
...
function makeCollectionRecord(
	...
	const derivedId =
		String(data?.id || "").trim() ||
		slugify(fileName.replace(/\.[^.]+$/, ""), `${workspace}-collection`);
	const title =
		String(
			data?.title || data?.name || derivedId || `${workspace} collection`,
		).trim() || `${workspace} collection`;
	const normalizedId = slugify(derivedId || title, `${workspace}-collection`);
	const ownerOrgId =
		String(
			data?.ownerOrganizationId ||
				data?.sourceOrganizationId ||
				organizationId ||
				"",
		).trim() || organizationId;
...
function ensureUniqueEntryId(list, candidate) {
	...
	const existing = new Set(
		(Array.isArray(list) ? list : [])
			.map((entry) =>
				isPlainObject(entry) ? String(entry.id || "").trim() : "",
			)
			.filter(Boolean),
	);
...
function mergeRelationOptions(target, source) {
	...
	for (const [key, value] of Object.entries(source || {})) {
		...
		const byId = new Map(
			existing.map((item) => [String(item.value), item]),
		);
		for (const item of Array.isArray(value) ? value : []) {
			byId.set(String(item.value), item);
		}
		merged[key] = Array.from(byId.values()).sort((a, b) =>
...
class OpenConfiguratorManagerElement extends HTMLElement {
	constructor() {
		...
		this.state = {
			...
			relationOptions: {},
			statusText:
				"Select an organization and open source files to begin.",
	...
	cacheDom() {
		...
		this.dom = {
			...
			confirmOrganizationBtn: root.getElementById(
				"confirmOrganizationBtn",
			),
			sourcesDialog: root.getElementById("sourcesDialog"),
			organizationSourceSummary: root.getElementById(
				"organizationSourceSummary",
			),
			productsSourceSummary: root.getElementById("productsSourceSummary"),
			materialsSourceSummary: root.getElementById(
				"materialsSourceSummary",
			),
			...
			setOrganizationSourceBtn: root.getElementById(
				"setOrganizationSourceBtn",
			),
			...
			newCollectionDialogTitle: root.getElementById(
				"newCollectionDialogTitle",
			),
			newCollectionNameInput: root.getElementById(
				"newCollectionNameInput",
			),
			confirmNewCollectionBtn: root.getElementById(
				"confirmNewCollectionBtn",
			),
		};
	}
	...
	bindEvents() {
		...
		this.dom.sectionBrowser.addEventListener(
			"section-open",
			async (event) => {
		...
		this.dom.organizationList?.addEventListener("click", (event) => {
			...
			const organizationId =
				button?.getAttribute("data-organization-id") || "";
			if (!organizationId) {
				return;
			}
		...
		this.dom.setOrganizationSourceBtn?.addEventListener(
			"click",
			async () => {
		...
		this.dom.newCollectionNameInput?.addEventListener("input", (event) => {
			this.state.pendingNewCollectionName = String(
				event.target?.value || "",
			);
			this.syncNewCollectionDialogControls();
		});
		this.dom.newCollectionNameInput?.addEventListener(
			"keydown",
			(event) => {
				if (event.key !== "Enter") {
					return;
	...
	organizationsFromManufacturerSource() {
		...
		if (
			Array.isArray(source?.discoveredOrganizations) &&
			source.discoveredOrganizations.length > 0
		) {
			return source.discoveredOrganizations
				.map((entry) => ({
					id: String(entry.id || entry.folderName || "").trim(),
					label:
						String(
							entry.label || entry.folderName || entry.id || "",
						).trim() || "Organization",
				}))
				.filter((entry) => entry.id);
		}
		const raw = Array.isArray(source?.data?.organizations)
		...
		const normalized = raw
			.map((entry, index) => {
				...
				const id =
					String(entry.id || entry.organizationId || "").trim() ||
					slugify(entry.label || entry.name || `org-${index + 1}`);
				const label =
					String(entry.label || entry.name || id).trim() || id;
				return { id, label };
			})
			.filter(Boolean);
		return normalized;
	...
	organizationOptions() {
		...
		return Array.isArray(this.state.organizations)
	...
	renderOrganizationDialog() {
		...
		if (organizations.length === 0) {
			this.dom.organizationList.innerHTML =
				'<div class="empty-state">No organizations available.</div>';
			return;
		}
		const pending =
			this.state.pendingOrganizationId || this.currentOrganizationId();
		this.dom.organizationList.innerHTML = organizations
			.map((org) => {
	...
	async confirmOrganizationSelection() {
		...
		try {
			...
			this.setStatus(
				`Organization switched to ${this.currentOrganizationLabel()}.`,
			);
		} catch (error) {
			this.setStatus(
				`Could not load organization info: ${error.message}`,
			);
		}
	...
	sourceSummary(source) {
		...
		const bits = [
			source.label || source.fileName || source.sourceId || "Source",
		];
		if (source.fileName) {
	...
	renderSourcesDialog() {
		...
		if (this.dom.organizationSourceSummary) {
			const discoveredCount = Array.isArray(
				manufacturer?.discoveredOrganizations,
			)
			...
			if (manufacturer?.connectionType === "local-folder") {
				...
				this.dom.organizationSourceSummary.textContent =
					this.sourceSummary(manufacturer);
			}
		}
		if (this.dom.productsSourceSummary) {
			...
			if (productsRoot?.connectionType === "local-folder") {
				...
				this.dom.productsSourceSummary.textContent =
					this.sourceSummary(productsRoot);
			}
		}
		...
		if (this.dom.packagesSourceSummary) {
			this.dom.packagesSourceSummary.textContent =
				this.sourceSummary(packages);
		}

		if (this.dom.setOrganizationSourceBtn) {
			this.dom.setOrganizationSourceBtn.textContent = manufacturer
				? "Replace source"
				: "Set source";
		}
		if (this.dom.setProductsSourceBtn) {
			this.dom.setProductsSourceBtn.textContent = productsRoot
				? "Replace source"
				: "Set source";
		}
		if (this.dom.setMaterialsSourceBtn) {
			this.dom.setMaterialsSourceBtn.textContent = materialsRoot
				? "Replace source"
				: "Set source";
		}
		if (this.dom.setPackagesSourceBtn) {
			this.dom.setPackagesSourceBtn.textContent = packages
				? "Replace source"
				: "Set source";
		}
	}
	...
	closeDialog(dialogId) {
		const dialog =
			this.dom?.[dialogId] || this.shadowRoot.getElementById(dialogId);
		if (dialog?.open) {
			dialog.close();
		}
	}
	...
	openNewCollectionDialog(workspace) {
		if (
			!this.dom.newCollectionDialog ||
			!this.dom.newCollectionDialogTitle
		) {
			return;
		}
		this.state.pendingNewCollectionWorkspace =
			workspace === "materials" ? "materials" : "products";
		this.state.pendingNewCollectionName = "";
		this.dom.newCollectionDialogTitle.textContent =
			this.state.pendingNewCollectionWorkspace === "materials"
	...
	serializeSourceForPersistence(source) {
		...
		return {
			...
			discoveredOrganizations: this.minimalDiscoveredOrganizations(
				source.discoveredOrganizations,
			),
			discoveredCollections: this.minimalDiscoveredCollections(
	...
	buildWorkspaceSnapshot() {
		return {
			...
			activeProductSourceId:
				this.state.workspace.activeProductSourceId || "",
			activeMaterialSourceId:
				this.state.workspace.activeMaterialSourceId || "",
			organizations: Array.isArray(this.state.organizations)
				? this.state.organizations
				: [],
			sources: {
				organization: this.serializeSourceForPersistence(
					this.manufacturerSource(),
				),
				products: this.serializeSourceForPersistence(
					this.productSource(),
				),
				materialsRoot: this.serializeSourceForPersistence(
					this.materialRootSource(),
				),
				materials: this.materialSources()
					.map((entry) => this.serializeSourceForPersistence(entry))
					.filter(Boolean),
				packages: this.serializeSourceForPersistence(
					this.packagesSource(),
				),
			},
		};
	...
	async restorePersistedWorkspaceState() {
		try {
			...
			this.state.workspace.currentOrganizationId =
				String(snapshot.currentOrganizationId || "").trim() ||
				this.state.workspace.currentOrganizationId;
			this.state.workspace.activeProductSourceId =
				String(snapshot.activeProductSourceId || "").trim() || null;
			this.state.workspace.activeMaterialSourceId =
				String(snapshot.activeMaterialSourceId || "").trim() || null;
			...
			const sources =
				snapshot.sources && typeof snapshot.sources === "object"
					? snapshot.sources
					: {};
			this.state.workspace.sources.manufacturer =
				this.inflateSourceFromPersistence(sources.organization);
			this.state.workspace.sources.products =
				this.inflateSourceFromPersistence(sources.products);
			this.state.workspace.sources.materialsRoot =
				this.inflateSourceFromPersistence(sources.materialsRoot);
			this.state.workspace.sources.packages =
				this.inflateSourceFromPersistence(sources.packages);
			this.state.workspace.sources.materials = Array.isArray(
				sources.materials,
			)
				? sources.materials
						.map((entry) =>
							this.inflateSourceFromPersistence(entry),
						)
						.filter(Boolean)
				: [];
	...
	async restoreSourceConnectionsAfterLoad() {
		...
		if (manufacturer) {
			if (manufacturer.connectionType === "local-folder") {
				const hasPermission = await this.hasHandleReadPermission(
					manufacturer.folderHandle,
				);
				if (!hasPermission) {
					this.markSourceReconnect(
						manufacturer,
					...
					const discovery =
						await this.discoverOrganizationsFromRootFolder(
							manufacturer.folderHandle,
						);
					manufacturer.discoveredOrganizations =
						discovery.organizations;
					if (discovery.organizations.length > 0) {
						this.state.organizations = discovery.organizations.map(
							(entry) => ({ id: entry.id, label: entry.label }),
						);
						if (
							!this.state.organizations.some(
								(entry) =>
									entry.id === this.currentOrganizationId(),
							)
						) {
							this.state.workspace.currentOrganizationId =
				...
				if (!hasPermission) {
					...
					try {
						...
						manufacturer.data = isPlainObject(parsed)
							? parsed
							: null;
						manufacturer.validation = validateDataRoot(
							"general",
							manufacturer.data,
						);
						this.clearSourceReconnect(manufacturer);
			...
			if (productsRoot.connectionType === "local-folder") {
				...
				if (!hasPermission) {
					...
					try {
						...
						const parsed = JSON.parse(await file.text());
						productsRoot.data = isPlainObject(parsed)
							? parsed
							: null;
						productsRoot.validation = validateDataRoot(
							"products",
							productsRoot.data,
						);
						this.clearSourceReconnect(productsRoot);
	...
	productSources() {
		...
		if (source.connectionType === "local-folder") {
			return Array.isArray(source.discoveredCollections)
				? source.discoveredCollections
				: [];
		}
		return [source];
	...
	selectedProductSource() {
		...
		return (
			sources.find(
				(entry) => entry.sourceId === activeId || entry.id === activeId,
			) || sources[0]
		);
	}
	...
	sectionListForData(data, validation = createEmptyValidation()) {
		...
		const orderMap = new Map(
			order.map((key, index) => [normalizeKey(key), index]),
		);
		const keys = Object.keys(data).filter((key) =>
			isEditableSection(data[key]),
		);
		keys.sort((a, b) => {
			const ai = orderMap.has(normalizeKey(a))
				? orderMap.get(normalizeKey(a))
				: Number.MAX_SAFE_INTEGER;
			const bi = orderMap.has(normalizeKey(b))
				? orderMap.get(normalizeKey(b))
				: Number.MAX_SAFE_INTEGER;
			if (ai !== bi) {
				return ai - bi;
			}
	...
	recomputeDerivedState() {
		...
		if (products) {
			if (products.connectionType === "local-folder") {
				const discovered = Array.isArray(products.discoveredCollections)
					? products.discoveredCollections
					: [];
				...
				products.validation = validateDataRoot(
					"products",
					products.data,
				);
		...
		this.state.workspace.sources.materials = this.materialSources().map(
			(source) => {
				const next = {
					...source,
		...
		if (this.materialRootSource()?.connectionType === "local-folder") {
			this.state.workspace.sources.materialsRoot.discoveredCollections =
				this.state.workspace.sources.materials;
		}
	...
	rebuildRelationOptions() {
		let options = {};
		options = mergeRelationOptions(
			options,
			buildRelationOptionsFromData(this.manufacturerSource()?.data),
		);

		for (const source of this.productSources()) {
			options = mergeRelationOptions(
				options,
				buildRelationOptionsFromData(source.data),
			);
		}
	...
	refreshExportSnapshot() {
		...
		const manufacturer = this.manufacturerSource();
		const withOrg = isPlainObject(manufacturer?.data)
			? mergeAdditive(base, manufacturer.data)
			: base;
		let merged = withOrg;
		...
		for (const [sectionId, sectionValidation] of Object.entries(
			arrayValidation,
		)) {
			if (sectionValidation.missingIds.length > 0) {
				warnings.push(
					`${friendlySectionLabel(sectionId)} has entries missing id.`,
				);
			}
			if (sectionValidation.duplicateIds.length > 0) {
				warnings.push(
					`${friendlySectionLabel(sectionId)} has duplicate ids: ${sectionValidation.duplicateIds.join(", ")}`,
				);
			}
		}
		...
		for (const [sectionId, sectionValue] of Object.entries(merged)) {
			...
			sectionValue.forEach((entry, index) => {
				...
				for (const [fieldName, targetSection] of Object.entries(
					RELATION_FIELD_TARGETS,
				)) {
						...
						for (const item of value) {
							...
							if (id && !targetIds.has(id)) {
								relationWarnings.push(
									`${sectionId}[${index}] ${fieldName} -> missing ${targetSection}:${id}`,
								);
							}
						}
					} else {
		...
		this.state.workspace.exportWarnings = [
			...warnings,
			...Array.from(new Set(relationWarnings)).slice(0, 120),
		];
	}
	...
	normalizeSelection() {
		if (this.state.currentLevel.startsWith("products-")) {
			...
			const activeProductSourceId =
				this.state.workspace.activeProductSourceId;
			const exists = products.some(
				(entry) =>
					entry.sourceId === activeProductSourceId ||
					entry.id === activeProductSourceId,
			);
			if (!exists) {
				this.state.workspace.activeProductSourceId =
					products[0]?.sourceId || products[0]?.id || null;
			}
			if (
				!this.state.workspace.activeProductSourceId &&
				this.state.currentLevel !== "products-collections"
		...
		if (this.state.currentLevel.startsWith("materials-")) {
			const materials = this.materialSources();
			const activeMaterialSourceId =
				this.state.workspace.activeMaterialSourceId;
			const exists = materials.some(
				(entry) =>
					entry.sourceId === activeMaterialSourceId ||
					entry.id === activeMaterialSourceId,
			);
			if (!exists) {
				this.state.workspace.activeMaterialSourceId =
					materials[0]?.sourceId || materials[0]?.id || null;
			}
			if (
				!this.state.workspace.activeMaterialSourceId &&
				this.state.currentLevel !== "materials-collections"
		...
		if (
			this.state.currentLevel.endsWith("sections") ||
			this.state.currentLevel.endsWith("collections")
		) {
			if (list.length === 0) {
				this.state.selectedEntryRef = null;
				return;
			...
			if (
				...
				this.state.workspace.activeProductSourceId =
					list[resolvedIndex].sourceId || list[resolvedIndex].id;
			}
			if (
				this.state.currentLevel === "materials-collections" &&
				Number.isInteger(resolvedIndex) &&
	...
	sectionEntriesForCurrentLevel() {
		if (this.state.currentLevel === "general-sections") {
			const manufacturer = this.manufacturerSource();
			return this.sectionListForData(
				manufacturer?.data,
	...
	contextActions() {
		if (this.state.currentLevel === "general-sections") {
			return [
				{ id: "open-manufacturer", label: "Open" },
				{
					...
					label: "Save",
					variant: "primary",
					disabled: !this.manufacturerSource()?.data,
				},
			];
		}
		...
		if (this.state.currentLevel === "products-collections") {
			return [
				...
				{
					id: "save-product-collection",
					label: "Save",
					variant: "primary",
					disabled: !this.selectedCollection(),
				},
			];
		}
		...
		if (this.state.currentLevel === "products-export") {
			return [
				...
				},
				{
					...
					disabled:
						!isPlainObject(this.workspaceGeneratedPackage()) ||
						Object.keys(this.workspaceGeneratedPackage()).length ===
							0,
				},
			];
		}
	...
	browserModel() {
		...
		if (this.state.currentLevel === "general-sections") {
			...
			model.overviewWarnings =
				manufacturer?.validation?.overviewWarnings || [];
		...
		if (this.state.currentLevel === "products-collections") {
			...
			model.sectionValue = sources.map((source) => ({
				...
				count: Object.keys(source.data || {}).filter((key) =>
					isEditableSection(source.data?.[key]),
				).length,
				warningCount: Number(
					source.validation?.overviewWarnings?.length || 0,
				),
				metaText: [
					source.fileName || "",
		...
		if (this.state.currentLevel === "products-export") {
			...
			model.exportPayload = {
				...
				jsonText: JSON.stringify(
					this.workspaceGeneratedPackage() || {},
					null,
					2,
				),
			};
			model.overviewWarnings = this.workspaceExportWarnings();
			model.sourceMeta = this.sourceMetaText(
		...
		if (
			...
			model.section = {
				...
				label: friendlySectionLabel(
					this.state.activeSectionId || "Section",
				),
				kind,
			};
	...
	exportSummaryStats() {
		...
		for (const [key, value] of Object.entries(generated)) {
			if (Array.isArray(value)) {
				stats.push({
					label: friendlySectionLabel(key),
					count: value.length,
				});
			}
		}
	...
	renderInspector() {
		...
		const section = {
			...
			kind: Array.isArray(this.currentSectionValue())
				? "array"
				: "object",
		};
		...
		const entryWarnings = (() => {
			if (
				...
				!Array.isArray(this.currentSectionValue())
			) {
				return [];
			}
			const index = Number(this.state.selectedEntryRef?.index);
			const map =
				context.owner.validation.sections?.[this.state.activeSectionId]
					?.entryWarnings || {};
	...
	async openSelectedCollection() {
		await this.openCollectionAtIndex(
			Number(this.state.selectedEntryRef?.index),
		);
	}
	...
	async openCollectionAtIndex(index) {
		...
		if (this.state.activeWorkspace === "products") {
			...
			await this.reloadProductCollectionFromIndex(
				selected.sourceId || selected.id,
			);
			this.state.currentLevel = "products-sections";
		} else if (this.state.activeWorkspace === "materials") {
			this.state.workspace.activeMaterialSourceId =
				selected.sourceId || selected.id;
			await this.reloadMaterialCollectionFromSettings(
				selected.sourceId || selected.id,
			);
			this.state.currentLevel = "materials-sections";
		}
	...
	createNewCollection(workspace, name = "") {
		const resolvedName =
			String(name || "").trim() ||
			`New ${workspace === "products" ? "product" : "material"} collection`;
		...
		const existingIds = new Set(
			this.currentWorkspaceCollections()
				.map((entry) =>
					String(entry.collectionId || entry.id || "").trim(),
				)
				.filter(Boolean),
		);
		let uniqueId = baseId;
		...
		if (workspace === "products") {
			...
			if (productsRoot?.connectionType === "local-folder") {
				...
				this.state.workspace.activeProductSourceId =
					collection.sourceId || collection.id;
			}
			this.state.activeWorkspace = "products";
			this.state.currentLevel = "products-sections";
		} else {
	...
	async openOrganizationSourceFolder() {
		try {
			...
			const discovery =
				await this.discoverOrganizationsFromRootFolder(rootHandle);
			const source = createSourceDescriptor({
			...
			if (discovery.organizations.length === 0) {
				...
				this.state.workspace.currentOrganizationId = "";
				this.setStatus(
					"Organization source set, but no valid organization folders were found.",
				);
				this.refreshAll();
				return;
			}
			...
			const current = this.currentOrganizationId();
			const nextId = discovery.organizations.some(
			...
			if (discovery.warnings.length > 0) {
				this.setStatus(
					`Organization source set (${discovery.organizations.length} orgs, ${discovery.warnings.length} warning${discovery.warnings.length === 1 ? "" : "s"}).`,
				);
			} else {
				this.setStatus(
					`Organization source set. Discovered ${discovery.organizations.length} organization${discovery.organizations.length === 1 ? "" : "s"}.`,
				);
			}
			this.refreshAll();
		} catch (error) {
	...
	async openProductsSourceFolder() {
		try {
			...
			const discoveredCount = Array.isArray(source.discoveredCollections)
				? source.discoveredCollections.length
				: 0;
			if (discoveredCount > 0) {
				this.setStatus(
					`Products source set. Discovered ${discoveredCount} product collection${discoveredCount === 1 ? "" : "s"} for ${this.currentOrganizationId() || "current org"}.`,
				);
			} else if (!this.currentOrganizationId()) {
				this.setStatus(
					"Products source set. Select an organization to resolve product collections.",
				);
			}
			this.state.activeWorkspace = "products";
			this.state.currentLevel = "products-collections";
	...
	async openMaterialsSourceFolder() {
		try {
			...
			if (discoveredCount > 0) {
				this.setStatus(
					`Materials source set. Discovered ${discoveredCount} material collection${discoveredCount === 1 ? "" : "s"} for ${this.currentOrganizationId() || "current org"}.`,
				);
			} else if (!this.currentOrganizationId()) {
				this.setStatus(
					"Materials source set. Select an organization to resolve material collections.",
				);
			}
			this.state.activeWorkspace = "materials";
			this.state.currentLevel = "materials-collections";
	...
	async resolveProductsSourceForCurrentOrganization() {
		...
		if (
			!source.folderHandle ||
			typeof source.folderHandle.getDirectoryHandle !== "function"
		) {
			source.discoveredCollections = [];
			this.state.workspace.activeProductSourceId = null;
			return;
		...
		try {
			orgDirectoryHandle =
				await source.folderHandle.getDirectoryHandle(orgDirectoryName);
		} catch (error) {
			if (error?.name === "NotFoundError") {
				...
				this.setStatus(
					`Products source does not contain ${orgDirectoryName}.`,
				);
				return;
			}
			throw error;
		...
		for await (const [
			...
			try {
				...
				const collectionId = slugify(
					String(parsed.id || "").trim() || productDirName,
					productDirName,
				);
				const title =
					String(
						parsed.title || parsed.name || parsed.label || "",
					).trim() || productDirName;
		...
		discoveredCollections.sort((a, b) =>
			String(a.title || a.id).localeCompare(String(b.title || b.id)),
		);
		source.discoveredCollections = discoveredCollections;

		const activeExists = discoveredCollections.some(
			(entry) =>
				entry.sourceId === previousActive ||
				entry.id === previousActive,
		);
		this.state.workspace.activeProductSourceId = activeExists
			? previousActive
			: discoveredCollections[0]?.sourceId ||
				discoveredCollections[0]?.id ||
				null;

		if (warningCount > 0) {
			this.setStatus(
				`Products source resolved ${discoveredCollections.length} collection${discoveredCollections.length === 1 ? "" : "s"} with ${warningCount} warning${warningCount === 1 ? "" : "s"}.`,
			);
		}
	}
	...
	async resolveMaterialsSourceForCurrentOrganization() {
		...
		try {
			suppliersDirectoryHandle =
				await source.folderHandle.getDirectoryHandle("suppliers");
		} catch (error) {
			if (error?.name === "NotFoundError") {
				source.discoveredCollections = [];
				source.resolvedOrgDirectoryName = "";
		...
		try {
			orgDirectoryHandle =
				await suppliersDirectoryHandle.getDirectoryHandle(orgId);
		} catch (error) {
			if (error?.name === "NotFoundError") {
				source.discoveredCollections = [];
				...
				this.setStatus(
					`Materials source does not contain suppliers/${orgId}.`,
				);
				return;
			}
			throw error;
		...
		for await (const [
			...
			try {
				settingsFileHandle =
					await entryHandle.getFileHandle("settings.json");
			} catch (error) {
				if (error?.name !== "NotFoundError") {
					warningCount += 1;
				}
			...
			try {
				...
				const collectionId = slugify(
					String(parsed.id || "").trim() || collectionDirName,
					collectionDirName,
				);
				const title =
					String(
						parsed.title || parsed.name || parsed.label || "",
					).trim() || collectionDirName;
		...
		if (warningCount > 0) {
			this.setStatus(
				`Materials source resolved ${discoveredCollections.length} collection${discoveredCollections.length === 1 ? "" : "s"} with ${warningCount} warning${warningCount === 1 ? "" : "s"}.`,
			);
		}
	}
	...
	async reloadProductCollectionFromIndex(productSourceId) {
		...
		const list = Array.isArray(sourceRoot.discoveredCollections)
			? sourceRoot.discoveredCollections
			: [];
		const index = list.findIndex(
			(entry) =>
				entry.sourceId === productSourceId ||
				entry.id === productSourceId,
		);
		if (index < 0) {
			return;
		}
		...
		if (
			!fileHandle &&
			target.collectionDirectoryHandle &&
			typeof target.collectionDirectoryHandle.getFileHandle === "function"
		) {
			fileHandle =
				await target.collectionDirectoryHandle.getFileHandle(
					"index.json",
				);
		}
		if (!fileHandle) {
		...
		try {
			...
			sourceRoot.discoveredCollections = list.map((entry, entryIndex) =>
				entryIndex === index ? next : entry,
			);
		} catch (error) {
			this.setStatus(
				`Failed to load ${target.fileName || "index.json"}: ${error.message}`,
			);
		}
	}
	...
	async reloadMaterialCollectionFromSettings(materialSourceId) {
		...
		const index = list.findIndex(
			(entry) =>
				entry.sourceId === materialSourceId ||
				entry.id === materialSourceId,
		);
		if (index < 0) {
			return;
		}
		...
		try {
			...
			const nextTitle =
				...
				target.collectionDirectoryName ||
				target.title ||
				target.id;
			const next = {
				...target,
			...
			this.state.workspace.sources.materials = list.map(
				(entry, entryIndex) => (entryIndex === index ? next : entry),
			);
			sourceRoot.discoveredCollections =
				this.state.workspace.sources.materials;
		} catch (error) {
			this.setStatus(
				`Failed to load ${target.fileName || "settings.json"}: ${error.message}`,
			);
		}
	}
	...
	async discoverOrganizationsFromRootFolder(rootHandle) {
		...
		for (const [entryName, entryHandle] of entries) {
			...
			try {
				...
				if (!isPlainObject(parsed)) {
					warnings.push(
						`${folderName}: ${infoFileName} must contain a JSON object.`,
					);
					continue;
				}
				const label =
				...
				warnings.push(
					`${folderName}: invalid ${infoFileName} (${error.message}).`,
				);
			}
		}
	...
	async loadOrganizationInfoForCurrentSelection() {
		...
		if (
			!source ||
			source.connectionType !== "local-folder" ||
			!selectedId
		) {
			return false;
		}

		const discovered = Array.isArray(source.discoveredOrganizations)
			? source.discoveredOrganizations
			: [];
		...
		if (
			...
			const dirHandle = await source.folderHandle.getDirectoryHandle(
				match.folderName,
			);
	...
	async handleOpenedFile(file, handle) {
		...
		try {
			...
			if (!isPlainObject(parsed)) {
				throw new Error(
					"Selected JSON must contain one top-level object.",
				);
			}
			...
			if (pending.type === "manufacturer") {
				const manufacturer = createSourceDescriptor({
					...
					fileHandle: handle,
					fileName:
						file.name || handle?.name || "manufacturer_info.json",
				...
				this.state.selectedEntryRef = null;
				this.setStatus(
					`Opened manufacturer source ${manufacturer.fileName}.`,
				);
			}
			...
			if (pending.type === "collection") {
				...
				if (workspace === "products") {
					...
					if (
						previous &&
						previous.collectionId === collection.collectionId
					) {
					...
					if (existingIndex >= 0) {
						collection.sourceId =
							next[existingIndex].sourceId || collection.sourceId;
				...
				this.state.selectedEntryRef = null;
				this.setStatus(
					`Opened ${workspace} source ${collection.title}.`,
				);
			}
			...
			if (pending.type === "packages") {
				...
				this.setStatus(
					`Opened packages source ${packagesSource.fileName}.`,
				);
			}
	...
	async saveManufacturer() {
		...
		try {
			...
			if (
				manufacturer.fileHandle &&
				typeof manufacturer.fileHandle.createWritable === "function"
			) {
				await this.writeToHandle(manufacturer.fileHandle, text);
			} else {
				await this.saveTextAsFile(
					text,
					manufacturer.fileName || "manufacturer_info.json",
				);
			}
	...
	async saveSelectedCollection(workspace) {
		...
		try {
			...
			if (
				collection.fileHandle &&
				typeof collection.fileHandle.createWritable === "function"
			) {
				await this.writeToHandle(collection.fileHandle, text);
			} else {
				await this.saveTextAsFile(
					text,
					collection.fileName || `${workspace}-collection.json`,
				);
			}
			setSourceDirty(collection, false);
			this.setStatus(
				`${workspace === "products" ? "Products" : "Materials"} source saved.`,
			);
			this.refreshAll();
		} catch (error) {
			if (error?.name === "AbortError") {
	...
	applyEntryChange(detail) {
		if (
			this.state.currentLevel === "general-sections" &&
			detail.sectionId === "__general_metadata"
		) {
			const source = this.manufacturerSource();
		...
		if (
			this.state.currentLevel === "products-sections" &&
			detail.sectionId === "__products_metadata"
		) {
			const source = this.selectedProductSource();
			if (!isPlainObject(source?.data) || !isPlainObject(detail.value)) {
				return;
		...
		if (
			this.state.currentLevel === "materials-sections" &&
			detail.sectionId === "__materials_metadata"
		) {
			const source = this.selectedMaterialSource();
			if (!isPlainObject(source?.data) || !isPlainObject(detail.value)) {
				return;
	...
	applyArrayAction(action) {
		...
		if (action === "add") {
			...
			if (
				!window.confirm(
					`Delete selected entry from ${friendlySectionLabel(sectionId)}?`,
				)
			) {
				return;
			}
			list.splice(index, 1);
			this.state.selectedEntryRef =
				list.length > 0
...
if (!customElements.get("open-configurator-manager")) {
	customElements.define(
		"open-configurator-manager",
		OpenConfiguratorManagerElement,
	);
}
