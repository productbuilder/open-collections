...
class OpenConfiguratorInspectorElement extends HTMLElement {
	...
	setData(data = {}) {
		...
		this.entryDraft =
			this.model.entryValue === undefined
				? undefined
				: cloneJson(this.model.entryValue);
		this.applyView();
	}
	...
	relationOptionsMarkup(target, selectedValues = [], includeBlank = false) {
		...
		const mapped = options
			.map((entry) => {
				const selected = selectedSet.has(String(entry.value))
					? "selected"
					: "";
	...
	bindCommonFieldEvents() {
		this.shadowRoot.querySelectorAll("[data-path]").forEach((node) => {
			...
			if (kind === "relation-multi") {
				node.addEventListener("change", () => {
					const values = Array.from(node.selectedOptions || [])
				...
				try {
					if (kind === "number") {
						...
						setByPath(
							this.entryDraft,
							path,
							this.parseJsonField(node.value),
						);
					} else {
	...
	bindObjectFieldEvents() {
		this.shadowRoot.querySelectorAll("[data-field-key]").forEach((node) => {
			...
			if (
				!key ||
				!this.entryDraft ||
				typeof this.entryDraft !== "object" ||
				Array.isArray(this.entryDraft)
			) {
				return;
			}
			...
			if (kind === "relation-multi") {
				node.addEventListener("change", () => {
					this.entryDraft[key] = Array.from(
	...
	bindConfigurationEvents() {
		...
		this.shadowRoot
			.querySelectorAll("[data-instance-remove-index]")
			.forEach((node) => {
				node.addEventListener("click", () => {
					const index = Number(
						node.getAttribute("data-instance-remove-index"),
					);
					if (
						!Number.isInteger(index) ||
						!Array.isArray(this.entryDraft.instances)
					) {
						return;
					}
		...
		this.shadowRoot
			.querySelectorAll("[data-instance-block-index]")
			.forEach((node) => {
				node.addEventListener("change", () => {
					const index = Number(
						node.getAttribute("data-instance-block-index"),
					);
					if (
						!Number.isInteger(index) ||
						!Array.isArray(this.entryDraft.instances) ||
						!this.entryDraft.instances[index]
					) {
						return;
					}
		...
		this.shadowRoot
			.querySelectorAll("[data-instance-json-index]")
			.forEach((node) => {
				node.addEventListener("change", () => {
					const index = Number(
						node.getAttribute("data-instance-json-index"),
					);
					if (
						!Number.isInteger(index) ||
						!Array.isArray(this.entryDraft.instances)
	...
	warningBlockMarkup() {
		const warnings = Array.isArray(this.model.entryWarnings)
			? this.model.entryWarnings
			: [];
	...
	objectFieldsMarkup(value) {
		...
		return keys
			.map((key) => {
				...
				const relationOptions = relationTarget
					? this.relationOptionsForTarget(relationTarget)
					: [];
	...
	blockEditorMarkup() {
		const entry =
			this.entryDraft && typeof this.entryDraft === "object"
				? this.entryDraft
				: {};
		...
		const height =
			getByPath(entry, "dimensions.height") ?? entry.height ?? "";
		const depth = getByPath(entry, "dimensions.depth") ?? entry.depth ?? "";
		const categoryIds = Array.isArray(entry.categoryIds)
			? entry.categoryIds.map((value) => String(value))
			: [];
		const materialIds = Array.isArray(entry.materialIds)
			? entry.materialIds.map((value) => String(value))
			: [];
		const meshIds = Array.isArray(entry.meshIds)
			? entry.meshIds.map((value) => String(value))
			: [];
	...
	configurationEditorMarkup() {
		const entry =
			this.entryDraft && typeof this.entryDraft === "object"
				? this.entryDraft
				: {};
		const blockIds = Array.isArray(entry.blockIds)
			? entry.blockIds.map((value) => String(value))
			: [];
		const instances = Array.isArray(entry.instances) ? entry.instances : [];
		const instanceMarkup = instances
			.map(
				(instance, index) => `
...
if (!customElements.get("open-configurator-inspector")) {
	customElements.define(
		...
		OpenConfiguratorInspectorElement,
	);
}
