...
class OpenConfiguratorSectionBrowserElement extends HTMLElement {
	...
	bindEvents() {
		...
		this.shadowRoot
			.getElementById("duplicateEntryBtn")
			?.addEventListener("click", () => {
		...
		this.shadowRoot
			.getElementById("moveEntryUpBtn")
			?.addEventListener("click", () => {
				this.dispatch("array-action", { action: "move-up" });
			});
		this.shadowRoot
			.getElementById("moveEntryDownBtn")
			?.addEventListener("click", () => {
	...
	renderContent() {
		...
		if (sectionKind === "array" && Array.isArray(this.model.sectionValue)) {
			...
			sectionKind === "object" &&
			this.model.sectionValue &&
			typeof this.model.sectionValue === "object"
		) {
		...
		if (this.model.sourceMeta) {
			subtitle = subtitle
				? `${subtitle} | ${this.model.sourceMeta}`
				: this.model.sourceMeta;
		}
		...
		const showToggle =
			sectionKind === "array" && this.model.showViewToggle !== false;
		...
		viewToggle.setAttribute(
			"mode",
			this.model.viewMode === "cards" ? "cards" : "rows",
		);

		if (actionsWrap) {
			actionsWrap.style.display =
				showToggle && hasAnyArrayActions ? "flex" : "none";
		...
		if (sectionKind === "array") {
			if (this.model.arrayPresentation === "section-nav") {
				host.innerHTML = this.renderSectionNavigationList(
					this.model.sectionValue,
				);
		...
		host.innerHTML =
			'<div class="empty">This section is not editable in this version.</div>';
	}
	...
	renderContextActions() {
		...
		const actions = Array.isArray(this.model.contextActions)
    ...
    `,
		...
		wrap.querySelectorAll("[data-context-action-id]").forEach((button) => {
			button.addEventListener("click", () => {
				const actionId =
					button.getAttribute("data-context-action-id") || "";
				if (actionId) {
	...
	renderOverview() {
		const stats = Array.isArray(this.model.overviewStats)
			? this.model.overviewStats
			: [];
		const overviewWarnings = Array.isArray(this.model.overviewWarnings)
			? this.model.overviewWarnings
			: [];
		if (stats.length === 0) {
	...
	renderExportPreview() {
		const payload =
			...
			typeof this.model.exportPayload === "object"
				? this.model.exportPayload
				: {};
		...
		const summaryCards =
			summary.length > 0
				? `<div class="overview-grid">${summary
						.map(
							(stat) => `
        <article class="overview-card">
          <p class="overview-label">${escapeHtml(stat.label)}</p>
	...
	renderWarnings() {
		...
		const lines = [];
		if (
			Array.isArray(sectionValidation.missingIds) &&
			sectionValidation.missingIds.length > 0
		) {
			lines.push(
				`Missing id in ${sectionValidation.missingIds.length} entr${sectionValidation.missingIds.length === 1 ? "y" : "ies"}.`,
			);
		}
		if (
			Array.isArray(sectionValidation.duplicateIds) &&
			sectionValidation.duplicateIds.length > 0
		) {
			lines.push(
				`Duplicate ids: ${sectionValidation.duplicateIds.join(", ")}`,
			);
		}
		if (lines.length === 0) {
			return "";
	...
	renderArrayRows(value) {
		...
		const rows = entries
			.map((entry, index) => {
				const selected =
					this.model.selectedEntryRef?.index === index
						? "is-selected"
						: "";
				...
				if (entry?.connectionType || entry?.sourceType) {
					sourceFlags.push(
						String(entry.connectionType || entry.sourceType),
					);
				}
				...
				const sourceDetail = [entry?.fileName, ...sourceFlags]
					.filter(Boolean)
					.join(" | ");
				return `
		...
		const bars = entries
			.map((entry, index) => {
				...
				const count = Number.isFinite(entry?.count)
					? entry.count
					: null;
				const warningCount = Number.isFinite(entry?.warningCount)
					? entry.warningCount
					: 0;
	...
	renderObjectRows(value) {
		...
		const rows = keys
			.map((key) => {
				const selected =
					this.model.selectedEntryRef?.key === key
						? "is-selected"
						: "";
	...
	bindSectionNavigationSelection() {
		this.shadowRoot
			.querySelectorAll(".section-nav-btn[data-entry-index]")
			.forEach((node) => {
				node.addEventListener("click", () => {
					const raw = node.getAttribute("data-entry-index");
					const index = Number(raw);
...
if (!customElements.get("open-configurator-section-browser")) {
	customElements.define(
		...
		OpenConfiguratorSectionBrowserElement,
	);
}
