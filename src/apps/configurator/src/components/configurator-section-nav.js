...
class OpenConfiguratorSectionNavElement extends HTMLElement {
	...
	render() {
		const domains = Array.isArray(this.model.domains)
			? this.model.domains
			: [];
      ...
      `;
		...
		const sections = Array.isArray(this.model.sections)
			? this.model.sections
			: [];
		const buttons = sections
			.map((section) => {
				const isActive = section.id === this.model.selectedSectionId;
				const count = Number.isFinite(section.count)
					? section.count
					: null;
				const warningCount = Number.isFinite(section.warningCount)
    ...
    `;
		...
		this.shadowRoot
			.querySelectorAll("button[data-domain-id]")
			.forEach((button) => {
				button.addEventListener("click", () => {
					const domainId =
						button.getAttribute("data-domain-id") || "manufacturer";
					this.dispatch("domain-select", { domainId });
				});
			});

		this.shadowRoot
			.querySelectorAll("button[data-section-id]")
			.forEach((button) => {
				button.addEventListener("click", () => {
					const sectionId =
						button.getAttribute("data-section-id") || "overview";
...
if (!customElements.get("open-configurator-section-nav")) {
	customElements.define(
		...
		OpenConfiguratorSectionNavElement,
	);
}
