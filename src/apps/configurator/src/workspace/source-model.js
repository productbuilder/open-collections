...
export function createSourceDescriptor(input = {}) {
	...
	return {
		...
		collectionId,
		connectionType:
			String(input.connectionType || "unknown").trim() || "unknown",
		...
		discoveredOrganizations: Array.isArray(input.discoveredOrganizations)
			? input.discoveredOrganizations
			: [],
		discoveredCollections: Array.isArray(input.discoveredCollections)
			? input.discoveredCollections
			: [],
		resolvedOrgDirectoryName: String(
			input.resolvedOrgDirectoryName || "",
		).trim(),
		...
		repo:
			input.repo && typeof input.repo === "object"
				? { ...input.repo }
				: null,
