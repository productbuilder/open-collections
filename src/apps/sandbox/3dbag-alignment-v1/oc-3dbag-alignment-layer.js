...
export class Oc3DBagAlignmentLayerElement extends HTMLElement {
	...
	_connectToMap() {
		...
		if (!this._map) {
			this._map = this.closest("main")?.querySelector("oc-map") || this.parentElement?.querySelector("oc-map") || document.querySelector("oc-map");
		}
		if (!this._map) {
			throw new Error("oc-3dbag-alignment-layer requires an oc-map in scope.");
		}
	...
	_onFeatureClick(event) {
		if (event?.detail?.layerId !== LAYER_FILL_ID && event?.detail?.layerId !== LAYER_LINE_ID) {
			return;
		}
	...
	async refresh({ force = false } = {}) {
		...
		// Rendering is intentionally isolated so this debug layer can later be
		// replaced by the same tile/mesh strategy as the official 3DBAG viewer.
		...
		this.dispatchEvent(
			new CustomEvent("oc-3dbag-layer-status", {
				...
				composed: true,
				detail: {
					strategy: "ogc-pand-items-fallback",
					bbox,
...
if (!customElements.get("oc-3dbag-alignment-layer")) {
	customElements.define("oc-3dbag-alignment-layer", Oc3DBagAlignmentLayerElement);
}
