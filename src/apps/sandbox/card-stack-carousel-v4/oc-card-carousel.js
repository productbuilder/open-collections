// src/apps/sandbox/card-stack-carousel-v4/oc-card-carousel.js
...
class OcCardCarousel extends HTMLElement {
	...
	addStepInputHandlers(maxStep, scrollPxPerCard) {
		...
		const goByStep = (delta) => {
			...
			this.emitActiveIndexChange(nextStep)
		...
		const goToTargetStep = () => {
			...
			this._wheelSnapTween = gsap.to(window, {
				...
				onComplete: () => {
					...
					this.emitActiveIndexChange(this._currentStep)
...
this._removeStepInputHandlers = () => {
	...
	emitActiveIndexChange(index) {
		const cardId = this.children[index]?.dataset?.id || this.children[index]?.getAttribute?.('data-id') || null
		this.dispatchEvent(new CustomEvent('oc-card-carousel-active-index-change', {
			detail: { index, cardId },
			bubbles: true,
			composed: true
		}))
