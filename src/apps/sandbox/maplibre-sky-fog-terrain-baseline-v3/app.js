...
const CARD_POSITION_TRANSFORMS = {
  leftFar: 'translate3d(calc(-50% - 270px), 10px, -180px) rotateY(34deg) scale(0.68)',
  left: 'translate3d(calc(-50% - 150px), 8px, -95px) rotateY(19deg) scale(0.84)',
  center: 'translate3d(-50%, -8px, 0) rotateY(0deg) scale(1)',
  right: 'translate3d(calc(-50% + 150px), 8px, -95px) rotateY(-19deg) scale(0.84)',
  rightFar: 'translate3d(calc(-50% + 270px), 10px, -180px) rotateY(-34deg) scale(0.68)'
};
...
function tuneBasemapLayerZoomConsistency() {
  ...
  console.info(
    '[maplibre-sky-fog-terrain-baseline-v3] basemap zoom consistency tuned',
    {
...
function getCardScenePosition(offset) {
  if (offset === 0) {
    return { transform: CARD_POSITION_TRANSFORMS.center, opacity: 1, zIndex: 7, blur: 0 };
  }
  if (offset === -1) {
    return { transform: CARD_POSITION_TRANSFORMS.left, opacity: 0.72, zIndex: 6, blur: 0 };
  }
  if (offset === 1) {
    return { transform: CARD_POSITION_TRANSFORMS.right, opacity: 0.72, zIndex: 6, blur: 0 };
  }
  if (offset === -2) {
    return { transform: CARD_POSITION_TRANSFORMS.leftFar, opacity: 0.32, zIndex: 4, blur: 1.2 };
  }
  if (offset === 2) {
    return { transform: CARD_POSITION_TRANSFORMS.rightFar, opacity: 0.32, zIndex: 4, blur: 1.2 };
  }
  return { transform: 'translate3d(-50%, 10px, -220px) scale(0.6)', opacity: 0, zIndex: 1, blur: 1.6 };
}

function createCardElement(card, index) {
  const element = document.createElement('article');
  element.className = 'top-carousel-card';
  element.dataset.index = String(index);
  element.innerHTML = `
    <span class="top-carousel-chip">${card.year}</span>
    <h3 class="top-carousel-title">${card.title}</h3>
    <div class="top-carousel-meta">${card.subtitle || 'Archive item'}</div>
    <div class="top-carousel-swatch" style="--swatch:${card.swatch || '#cfd8ea'}"></div>
  `;
  return element;
}

function setupTopCarousel() {
  const track = document.getElementById('top-carousel-track');
  ...
  function renderCarousel() {
    cardElements.forEach((element, index) => {
      const deltaRaw = index - activeIndex;
      const wrappedDelta = ((deltaRaw + CAROUSEL_CARDS.length + 3) % CAROUSEL_CARDS.length) - 3;
      const scene = getCardScenePosition(wrappedDelta);
