...
class OpenCollectionsTimeSliderV3AppElement extends HTMLElement {
	constructor() {
		super();
	...
	bindEvents() {
		const ruler = this.shadowRoot?.querySelector("oc-timeslider-v3-ruler");
		if (!ruler) {
			return;
		}
		ruler.addEventListener("focus-year-change", (event) => {
			const nextFocus = Number(event.detail?.focusYear);
			if (!Number.isFinite(nextFocus)) {
				return;
			}
	...
	applyView() {
		...
		if (!focusValue || !rangeValue || !widthValue || !domainValue) {
			return;
		}
			...
			<div class="app">
				<div class="page">
					<header class="header">
						...
						<div class="readouts">
							...
							<div class="readout">
								<strong>Range width</strong>
								<span id="widthValue">24 years</span>
							</div>
							<div class="readout">
								<strong>Domain model</strong>
								<span id="domainValue">1800 to 2025</span>
							</div>
						</div>
					</header>
				</div>
				<div class="footer-rail">
					<div class="ruler-stage">
						<oc-timeslider-v3-ruler></oc-timeslider-v3-ruler>
					</div>
				</div>
			</div>
		`;
...
if (!customElements.get("open-collections-timeslider-v3")) {
	customElements.define(
		"open-collections-timeslider-v3",
		OpenCollectionsTimeSliderV3AppElement,
	);
}

export { OpenCollectionsTimeSliderV3AppElement };
