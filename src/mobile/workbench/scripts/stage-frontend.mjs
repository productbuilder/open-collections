...
function stageIndexHtml() {
  const workbenchIndexPath = path.join(WORKBENCH_ROOT, 'index.html');
  ...
  const adjusted = original.replace(
    ...
    '<script type="module" src="./src/mobile/workbench/src/main.js"></script>',
  );
...
function stageFrontend() {
  ...
  const workbenchSource = path.join(WORKBENCH_ROOT, 'src');
  const workbenchDestination = path.join(distSrcRoot, 'mobile', 'workbench', 'src');
  ensureDir(path.dirname(workbenchDestination));
  copyPath(workbenchSource, workbenchDestination);

  stageIndexHtml();
  process.stdout.write(`Staged mobile frontend at ${DIST_ROOT}\n`);
}
