	...
	try {
		...
		if (/^\s*</.test(rawText)) {
			return {
				ok: false,
				message:
					"File is not publicly accessible or not shared as a direct JSON file.",
			};
		}
		return {
			ok: false,
			message:
				"Could not parse JSON manifest. Ensure the shared file is a valid collection.json.",
		};
	}
}
...
export function normalizeGoogleDriveManifestUrl(inputUrl) {
	...
	return {
		...
		variant: parsed.pathname.includes("/uc")
			? "uc-download"
			: "shared-link",
	};
}
...
export function createGoogleDriveProvider() {
	...
	const descriptor = createProviderDescriptor({
		...
		description:
			"Connect to Google Drive and browse shared or private collection sources.",
	...
	async function loadPublicManifest(config = {}) {
		...
		if (!normalized.ok) {
			...
			return disconnectWithError(
				normalized.message || "Invalid Google Drive file URL.",
				false,
			);
		}
		...
		if (!response.ok) {
			...
			return disconnectWithError(
				`Failed to load Google Drive file (${response.status}).`,
				false,
			);
		}
	...
	async function loadAuthenticatedManifest(config = {}) {
		...
		if (!DRIVE_FILE_ID_PATTERN.test(inputFileId)) {
			return disconnectWithError(
				"Enter a valid Google Drive file ID for collection.json.",
				true,
			);
		}
		...
		if (!accessToken) {
			return disconnectWithError(
				"Re-authentication required. Click Connect Google Drive.",
				true,
			);
		}
		...
		if (!metadataResponse.ok) {
			if ([401, 403].includes(metadataResponse.status)) {
				return disconnectWithError(
					"Google Drive authorization failed or permission denied.",
					true,
				);
			}
			if (metadataResponse.status === 404) {
				return disconnectWithError(
					"Google Drive file not found for the provided file ID.",
					true,
				);
			}
			return disconnectWithError(
		...
		if (!mediaResponse.ok) {
			...
			return disconnectWithError(
				`Google Drive file download failed (${mediaResponse.status}).`,
				true,
			);
		}
		...
		setState(CONNECTION_STATES.connected);
		const title =
			normalizeManifestTitle(collection.title) ||
			normalizeManifestTitle(metadata?.name);
		return {
			ok: true,
	...
	return {
		...
		async connect(config = {}) {
			...
			sourceMode =
				asTrimmedString(config.sourceMode) || "public-manifest-url";
			try {
				...
				return disconnectWithError(
					`Google Drive connection failed: ${error.message}`,
					sourceMode === "auth-manifest-file",
				);
			}
		},
