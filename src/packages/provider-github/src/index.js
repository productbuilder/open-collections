...
export function createGithubProvider() {
	...
	const descriptor = createProviderDescriptor({
		...
		description:
			"Connect to a GitHub repository using a Personal Access Token and load media files.",
	...
	async function fetchRepoContents(pathValue, options = {}) {
		...
		if (!response.ok) {
			...
			if (response.status === 401 || response.status === 403) {
				throw new Error(
					"Authentication failed or repository access denied.",
				);
			}
			...
			if (response.status === 404) {
				...
				throw new Error(
					`Repository or path not accessible: ${location}`,
				);
			}

			throw new Error(
				`GitHub API ${response.status}: ${errorBody.slice(0, 220)}`,
			);
		}
	...
	async function putRepoContent(pathValue, payload) {
		...
		if (!response.ok) {
			...
			if (response.status === 401 || response.status === 403) {
				throw new Error(
					"Save failed: insufficient permissions or invalid token.",
				);
			}
			if (response.status === 409 || response.status === 422) {
				throw new Error(
					"Save failed: collection.json changed upstream (SHA conflict). Refresh source and retry.",
				);
			}
			throw new Error(
				`Save failed: GitHub API ${response.status}: ${errorBody.slice(0, 220)}`,
			);
		}
	...
	async function decodeContentEntry(entry) {
		...
		if (entry.download_url) {
			...
			if (!response.ok) {
				throw new Error(
					`Failed to fetch manifest content (${response.status}).`,
				);
			}
		...
		throw new Error(
			"Unable to read collection.json content from GitHub response.",
		);
	}
	...
	function normalizeManifestItem(item, index, manifestFolderPath) {
		...
		const mediaType = (
			media.type ||
			mediaTypeForPath(media.url || "") ||
			mediaTypeForPath(mediaUrl || "") ||
			"image"
		).toLowerCase();
	...
	async function loadCollectionManifest(rootFolderPath) {
		...
		const manifestEntry = await fetchRepoContents(manifestPath, {
			allowNotFound: true,
		});
		if (
			!manifestEntry ||
		...
		try {
			...
			throw new Error(
				`collection.json could not be parsed: ${error.message}`,
			);
		}
		...
		if (validationErrors.length > 0) {
			throw new Error(
				`collection.json schema invalid: ${validationErrors.join(" ")}`,
			);
		}
			...
			if (child.type === "file") {
				...
				files.push({
					...
					media: {
						...
						url: rawUrl,
						thumbnailUrl:
							mediaType === "image" ? rawUrl : undefined,
					},
				});
			}
		}
	...
	return {
		...
		async connect(config = {}) {
			...
			if (!token || !owner || !repo) {
				...
				return {
					ok: false,
					message:
						"Enter token, owner, and repository to connect GitHub.",
					capabilities,
				};
			}
			...
			try {
				...
				const manifestResult =
					await loadCollectionManifest(contentPath);
				...
				const message = manifestResult.found
					? `Connected to GitHub repo ${where}. Found ${manifestResult.manifestPath}, loaded ${items.length} items, and ${
							capabilities.canSaveMetadata
								? "enabled metadata save."
								: "running in read-only mode (no repo write permission)."
						}`
		...
		async saveMetadata(id, patch) {
			...
			if (!capabilities.canSaveMetadata) {
				throw new Error(
					"Save failed: collection manifest is read-only for this GitHub source.",
				);
			}

			if (!collection || !Array.isArray(collection.items)) {
				throw new Error(
					"Save failed: no editable collection manifest loaded.",
				);
			}

			const targetIndex = collection.items.findIndex(
				(entry) => entry.id === id,
			);
			if (targetIndex === -1) {
				throw new Error(
					`Save failed: item not found in manifest (${id}).`,
				);
			}
			...
			if (!nextSha) {
				throw new Error(
					"Save failed: missing updated file SHA in GitHub response.",
				);
			}
			...
			items = (collection.items || []).map((entry, index) =>
				normalizeManifestItem(entry, index, manifestRootPath),
			);

			const updated = items.find((entry) => entry.id === id);
			if (!updated) {
				throw new Error(
					`Save failed: item not found after save (${id}).`,
				);
			}
		...
		async removeItemsFromCollection(itemIds = []) {
			...
			if (!capabilities.canSaveMetadata) {
				throw new Error(
					"Delete failed: collection manifest is read-only for this GitHub source.",
				);
			}
			if (!collection || !Array.isArray(collection.items)) {
				throw new Error(
					"Delete failed: no editable collection manifest loaded.",
				);
			}
			...
			const removableIds = new Set(
				collection.items.map((entry) => String(entry.id || "").trim()),
			);
			const nextItems = collection.items.filter(
				(entry) => !ids.includes(String(entry.id || "").trim()),
			);
			if (nextItems.length === collection.items.length) {
				throw new Error(
					"Delete failed: no matching manifest items were found.",
				);
			}
			...
			if (!nextSha) {
				throw new Error(
					"Delete failed: missing updated file SHA in GitHub response.",
				);
			}
		...
		async publishCollection(payload = {}) {
			...
			if (!token || !owner || !repo) {
				throw new Error(
					"Publish failed: connect GitHub with token, owner, and repository.",
				);
			}

			if (!hasWriteAccess) {
				throw new Error(
					"Publish failed: token does not have write access to this repository.",
				);
			}
			...
			if (validationErrors.length > 0) {
				throw new Error(
					`Publish failed: manifest validation errors: ${validationErrors.join(" ")}`,
				);
			}
			...
			for (const entry of uploads) {
				...
				const existing = await fetchRepoContents(path, {
					allowNotFound: true,
				});
				const nextPayload = {
					message:
						entry.message || `Upload ${path} via TimeMap Collector`,
			...
			const collectionRootPath = asApiPath(
			...
			const existingManifestEntry = await fetchRepoContents(
				targetManifestPath,
				{ allowNotFound: true },
			);
			...
			const manifestPayload = {
				message:
					...
					"Publish collection via TimeMap Collector",
				content: encodeBase64Utf8(serialized),
				branch,
			};
			if (
			...
			const manifestResponse = await putRepoContent(
				targetManifestPath,
				manifestPayload,
			);
			const nextSha = manifestResponse?.content?.sha || "";
			const manifestFolder = targetManifestPath.includes("/")
				? targetManifestPath.slice(
						0,
						targetManifestPath.lastIndexOf("/"),
					)
				: "";
