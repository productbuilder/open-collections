...
export function createLocalProvider() {
	...
	const descriptor = createProviderDescriptor({
		...
		description:
			"Connect a local host root folder and manage writable collections via the File System Access API.",
	...
	function normalizeCollectionRecord(manifest, options = {}) {
		const fallbackId =
			...
			"collection";
		const collectionId = collectionIdFromValue(
			manifest.id || fallbackId,
			"collection",
		);
		const collectionTitle =
			String(
				manifest.title || options.fallbackTitle || collectionId,
			).trim() || collectionId;
		const rootPath =
			String(
				options.rootPath ||
					manifest.rootPath ||
					`${options.directoryName || collectionId}/`,
			).trim() || `${options.directoryName || collectionId}/`;
		const directoryName = collectionIdFromValue(
			options.directoryName || rootPath.split("/")[0] || collectionId,
			collectionId,
		);
		const manifestPath =
			String(
				options.manifestPath || `${directoryName}/collection.json`,
			).trim() || `${directoryName}/collection.json`;
		...
		const normalizedManifest = {
			...
			items: Array.isArray(manifest.items)
				? manifest.items.map((item) => cloneItem(item))
				: [],
		};
	...
	function normalizeCollectionManifest(
		manifest,
		fallbackId,
		fallbackTitle,
		manifestUrl,
	...
	async function fetchJson(pathOrUrl, basePath = window.location.href) {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to load ${toRelativePath(targetUrl)} (${response.status}).`,
			);
		}
	...
	function ensureWritableLocalHost() {
		...
		if (!canWriteLocalHost()) {
			throw new Error(
				"Connected local source is read-only. Reconnect with a writable folder host.",
			);
		}
	}
	...
	async function writeBlobByRelativePath(
		directoryHandle,
		relativePath,
		blob,
	) {
		...
		const parentDir = parentPath
			? await getDirectoryByRelativePath(directoryHandle, parentPath, {
					create: true,
				})
			: directoryHandle;
	...
	async function loadCollectionRecordFromRelativeManifestPath(
		manifestPath,
		options = {},
	) {
		const relativePath = String(manifestPath || "")
			.trim()
			.replace(/^\/+/, "");
		if (!relativePath) {
		...
		if (parts.length < 2) {
			throw new Error(
				`Collection manifest path must include directory and file: ${relativePath}`,
			);
		}
		const fileName = parts.pop();
		const directoryPath = parts.join("/");
		const directoryHandle = await getDirectoryByRelativePath(
			rootDirectoryHandle,
			directoryPath,
			{ create: false },
		);
		...
		if (validationErrors.length > 0) {
			throw new Error(
				`Collection schema invalid for ${relativePath}: ${validationErrors.join(" ")}`,
			);
		}
		return normalizeCollectionRecord(manifest, {
			fallbackId:
				...
				"collection",
			fallbackTitle:
				options.fallbackTitle || options.fallbackId || "Collection",
			rootPath:
				options.rootPath || manifest.rootPath || `${directoryPath}/`,
			directoryName:
				options.directoryName ||
				directoryPath.split("/").filter(Boolean).pop() ||
				directoryPath,
		...
		if (indexCollections.length > 0) {
			for (const entry of indexCollections) {
				...
				const manifestPath = String(
					entry.manifest ||
						entry.collectionJsonPath ||
						(entry.id ? `${entry.id}/collection.json` : ""),
				).trim();
				if (!manifestPath) {
					continue;
				}
				const pathSegment = String(
					entry.path || entry.directory || entry.id || "",
				)
					.split("/")
					.filter(Boolean)
					.pop();
				...
				const manifestFolder =
					manifestParts.length > 1
						? manifestParts[manifestParts.length - 2]
						: "";
				const folderSegment = pathSegment || manifestFolder;
				const fallbackId = String(
					entry.id || folderSegment || "collection",
				).trim();
				try {
					const collection =
						await loadCollectionRecordFromRelativeManifestPath(
							manifestPath,
							{
								fallbackId,
	...
	function ensureUniqueItemId(collection, baseId) {
		const existing = new Set(
			(collection.manifest.items || []).map((entry) => entry.id),
		);
	...
	async function writeCollectionManifest(collection) {
		const directory = await getDirectoryByRelativePath(
			rootDirectoryHandle,
	...
	function providerItemIdFor(collectionId, manifestItemId) {
		for (const [providerItemId, location] of itemLocations.entries()) {
			if (
				...
				location.manifestItemId === manifestItemId
			) {
	...
	function rebuildIndex() {
		...
		for (const collection of collections) {
			const manifestItems = Array.isArray(collection.manifest?.items)
				? collection.manifest.items
				: [];
			for (let index = 0; index < manifestItems.length; index += 1) {
				const manifestItem = manifestItems[index];
				const manifestItemId =
					String(manifestItem.id || "").trim() ||
	...
	return {
		...
		async connect(config = {}) {
			sourcePath =
				String(
					config.path || sourcePath || DEFAULT_SOURCE_PATH,
				).trim() || DEFAULT_SOURCE_PATH;
			const sourceLabel =
				String(config.localDirectoryName || "").trim() ||
				hostLabelFromPath(sourcePath);
			const nextDirectoryHandle =
				config.localDirectoryHandle &&
				config.localDirectoryHandle.kind === "directory"
					? config.localDirectoryHandle
					: null;
			...
			try {
				...
				if (Array.isArray(json.collections)) {
					...
					for (const entry of json.collections) {
						...
						const manifestPath = String(
							entry.manifest || entry.collectionJsonPath || "",
						).trim();
						if (!manifestPath) {
							continue;
						}
						const { json: manifest, url: manifestUrl } =
							await fetchJson(manifestPath, url);
						const validationErrors =
							validateCollectionShape(manifest);
						if (validationErrors.length > 0) {
							throw new Error(
								`Collection schema invalid for ${manifestPath}: ${validationErrors.join(" ")}`,
							);
						}
				...
				if (validationErrors.length > 0) {
					throw new Error(
						`Collection schema invalid: ${validationErrors.join(" ")}`,
					);
				}
		...
		async saveMetadata(id, patch) {
			...
			const itemIndex = collection.manifest.items.findIndex(
				(entry) => entry.id === location.manifestItemId,
			);
			if (itemIndex === -1) {
				return null;
			}
		...
		async createCollection(collectionMeta = {}) {
			...
			const baseId = collectionIdFromValue(
				collectionMeta.id || collectionMeta.title || "collection",
				"collection",
			);
			const collectionId = ensureUniqueCollectionId(baseId);
			const directoryName = collectionId;

			const existingDirectory = await getDirectoryByRelativePath(
				rootDirectoryHandle,
				directoryName,
				{ create: true },
			);
			const existingManifestHandle = await tryGetFileHandle(
				existingDirectory,
				"collection.json",
			);
			if (existingManifestHandle) {
				throw new Error(
					`Collection directory ${directoryName} already contains collection.json.`,
				);
			}

			const rootPath =
				String(collectionMeta.rootPath || `${directoryName}/`).trim() ||
				`${directoryName}/`;
			const title =
				String(collectionMeta.title || collectionId).trim() ||
				collectionId;
		...
		async saveCollectionMetadata(collectionId, patch = {}) {
			...
			if (!collection) {
				throw new Error(
					`Collection ${collectionId} was not found in this local host.`,
				);
			}

			collection.title =
				String(patch.title ?? collection.title).trim() ||
				collection.title;
			collection.description = String(
				patch.description ?? collection.description,
			).trim();
			collection.license = String(
				patch.license ?? collection.license,
			).trim();
			collection.publisher = String(
				patch.publisher ?? collection.publisher,
			).trim();
			collection.language = String(
				patch.language ?? collection.language,
			).trim();
		...
		async removeItemsFromCollection(collectionId, itemIds = []) {
			...
			if (manifestIdsToRemove.size === 0) {
				throw new Error(
					`Delete failed: no matching items were found in collection ${collectionId}.`,
				);
			}
			...
			collection.manifest.items = (
				...
				(entry) =>
					!manifestIdsToRemove.has(String(entry.id || "").trim()),
			);
			...
			rebuildIndex();
			return {
				removedIds: ids.filter(
					(providerItemId) => !itemsById.has(providerItemId),
				),
			};
		},
		...
		async addAssetToCollection(collectionId, payload = {}) {
			...
			if (
				!originalFile ||
				typeof originalFile.arrayBuffer !== "function"
			) {
				throw new Error(
					"Cannot add asset: local file payload is missing.",
				);
			}

			const rawItemId = collectionIdFromValue(
				payload.itemId ||
					payload.id ||
					payload.title ||
					originalFile.name ||
			...
			const mediaRelativePath = String(
				payload.mediaPath ||
					`${itemId}${extensionForFileName(originalFile.name, ".jpg")}`,
			)
				.trim()
				.replace(/^\/+/, "");
			const thumbnailRelativePath = payload.thumbnailBlob
				? String(payload.thumbnailPath || `${itemId}.thumb.jpg`)
						.trim()
						.replace(/^\/+/, "")
				: "";

			await writeBlobByRelativePath(
				rootDirectoryHandle,
				`${collection.directoryName}/${mediaRelativePath}`,
				originalFile,
			);
			if (payload.thumbnailBlob && thumbnailRelativePath) {
				await writeBlobByRelativePath(
					rootDirectoryHandle,
					`${collection.directoryName}/${thumbnailRelativePath}`,
			...
			const item = {
				...
				source: String(
					payload.source ||
						`${collection.directoryName}/collection.json`,
				).trim(),
				...
				media: {
					mode: "managed",
					type:
						String(payload.mediaType || "image").trim() || "image",
			...
			collection.manifest.items = Array.isArray(collection.manifest.items)
				? collection.manifest.items
				: [];
			...
			const providerItemId =
				providerItemIdFor(collection.id, item.id) || item.id;
		...
		async readCollectionFileBlob(collectionId, relativePath) {
			...
			return readBlobByRelativePath(
				rootDirectoryHandle,
				`${collection.directoryName}/${filePath}`,
			);
		},
