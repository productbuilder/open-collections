...
async function signingKey(secretKey, shortDate, region, service = 's3') {
  const encoder = new TextEncoder();
  const kDate = await hmacSha256Raw(encoder.encode(`AWS4${secretKey}`), shortDate);
...
export function createS3Provider() {
  ...
  async function signedFetch({ method = 'GET', objectKey = '', query = {}, headers = {}, body = null } = {}) {
    ...
    const canonicalUri = encodedObjectKey ? `/${encodedBucket}/${encodedObjectKey}` : `/${encodedBucket}`;
    ...
    const canonicalHeaders = signedHeaders
      .map((name) => `${name}:${String(requestHeaders[name] || requestHeaders[name.toUpperCase()] || '').trim()}\n`)
      .join('');
    ...
    const key = await signingKey(configState.secretKey, shortDate, region, 's3');
  ...
  async function validateConnection() {
    try {
      ...
      if (response.status === 403 || response.status === 401) {
        throw new Error(`S3 connection failed: credentials are invalid or do not have access to bucket ${configState.bucket}. ${message}`.trim());
      }
      if (response.status === 404) {
        throw new Error(`S3 connection failed: bucket ${configState.bucket} was not found at ${configState.endpoint}.`);
      }
      throw new Error(`S3 connection failed (${response.status}): ${message || 'Unable to validate endpoint/bucket configuration.'}`);
    } catch (error) {
      if (error instanceof TypeError) {
        throw new Error(`S3 connection failed: could not reach ${configState.endpoint}. Check endpoint URL, CORS policy, and network access.`);
      }
      throw error;
    }
  }
  ...
  return {
    ...
    async publishCollection(payload = {}) {
      ...
      if (!configState.accessKey || !configState.secretKey) {
        throw new Error('Publish failed: credentials are missing. Reconnect this S3 host and provide access key + secret key.');
      }
      ...
      for (const entry of uploads) {
        ...
        if (!response.ok) {
          const responseText = await response.text();
          const { message } = parseS3ErrorXml(responseText);
          throw new Error(`Publish failed while uploading ${relativePath}: ${message || `S3 returned ${response.status}.`}`);
        }
      ...
      const collectionRootPath = joinKey(payload.collectionRootPath || '');
      const manifestKey = joinKey(configState.basePath, collectionRootPath, 'collection.json');
      ...
      if (!manifestResponse.ok) {
        ...
        const { message } = parseS3ErrorXml(responseText);
        throw new Error(`Publish failed while writing collection.json: ${message || `S3 returned ${manifestResponse.status}.`}`);
      }
