...
export function createDefaultConnectionProviderCatalog(providerFactories = createDefaultConnectionProviderFactories()) {
  return [
    {
...
export function sourceDetailLabelFor(providerId, config, fallbackLabel, defaultManifestPath) {
  if (providerId === 'github') {
...
export function sanitizeConnectionConfig(providerId, config = {}, defaultManifestPath) {
  if (providerId === 'github') {
    return {
...
export function sourceIdentityKey(source, defaultManifestPath) {
  ...
  const sanitized = sanitizeConnectionConfig(providerId, source.config || {}, defaultManifestPath);
...
export function toPersistedConnection(source, defaultManifestPath) {
  return {
    ...
    label: source.label,
    config: sanitizeConnectionConfig(source.providerId, source.config || {}, defaultManifestPath),
...
export function createConnectionsRuntime(options = {}) {
  const {
    ...
    providerCatalog = createDefaultConnectionProviderCatalog(providerFactories),
    credentialStore,
    makeConnectionId,
  ...
  return {
    ...
    providerFactories,
    providerCatalog,
    collectProviderConfig(providerId, rawConfig = {}, selectedLocalDirectoryHandle = null) {
      const config = { ...rawConfig };
      if (providerId === 'local') {
        config.localDirectoryName = (config.localDirectoryName || '').trim();
        const selectedHandle = selectedLocalDirectoryHandle && selectedLocalDirectoryHandle.kind === 'directory'
    ...
    async connectSource({ providerId, config, pendingRepairSource = null, sources = [] }) {
      const providerFactory = providers[providerId];
      const providerMeta = providerCatalog.find((entry) => entry.id === providerId);
      if (!providerFactory || providerMeta?.enabled === false) {
        return { ok: false, message: 'Selected connection type is not available yet.' };
      }
      ...
      const displayLabel = result.sourceDisplayLabel
        || sourceDisplayLabelFor(providerId, config, providerMeta?.label || providerId);
      const detailLabel = result.sourceDetailLabel
        || sourceDetailLabelFor(providerId, config, providerMeta?.label || providerId, defaultManifestPath);
      ...
      const duplicate = !pendingRepairSource
        ? sources.find((entry) => sourceIdentityKey(entry, defaultManifestPath) === sourceIdentityKey(draftSource, defaultManifestPath))
        : null;
      const target = duplicate || pendingRepairSource;
      const source = { ...draftSource, id: target?.id || draftSource.id };

      if ((providerId === 'github' || providerId === 's3') && credentialStore) {
    ...
    async refreshSource({ source, sources = [], configOverrides = {}, pendingSourceRepair = null, selectedLocalDirectoryHandle = null }) {
      ...
      if (!providerFactory) {
        return { ok: false, message: `Connection type for ${source.displayLabel || source.id} is unavailable.` };
      }
      const provider = providerFactory();
      let refreshConfig = { ...(source.config || {}), ...(configOverrides || {}) };

      if (source.providerId === 'github' && !(refreshConfig.token || '').trim() && credentialStore) {
        refreshConfig = await credentialStore.loadSourceSecret(source, refreshConfig);
      }
      if (source.providerId === 's3' && (!(refreshConfig.accessKey || '').trim() || !(refreshConfig.secretKey || '').trim()) && credentialStore) {
        refreshConfig = await credentialStore.loadSourceSecret(source, refreshConfig);
      }
      if (source.providerId === 'local') {
        const explicitHandle = configOverrides?.localDirectoryHandle;
        const repairHandle = pendingSourceRepair?.sourceId === source.id ? selectedLocalDirectoryHandle : null;
      ...
      if (!result.ok) {
        ...
        return {
          ...
          sources: sources.map((entry) => (entry.id === source.id ? failed : entry)),
        };
      }
      ...
      const updated = {
        ...
        itemCount: Array.isArray(loaded) ? loaded.length : 0,
        status: result.message || 'Connected',
        displayLabel: result.sourceDisplayLabel || sourceDisplayLabelFor(source.providerId, refreshConfig, source.providerLabel),
        detailLabel: result.sourceDetailLabel || sourceDetailLabelFor(source.providerId, refreshConfig, source.providerLabel, defaultManifestPath),
        needsReconnect: false,
        needsCredentials: false,
        capabilities: provider.getCapabilities(),
      };
    ...
    async removeSource({ sourceId, sources = [], activeSourceId = 'all' }) {
      ...
      return {
        ...
        sources: nextSources,
        activeSourceId: activeSourceId === sourceId ? (nextSources[0]?.id || 'all') : activeSourceId,
      };
    },
    persistSources(sources = []) {
      ...
      const payload = sources.map((source) => toPersistedConnection(source, defaultManifestPath));
      try {
    ...
    restoreRememberedSources() {
      ...
      return remembered
        ...
        .map((entry) => ({
          ...
          providerLabel: entry.providerLabel || providerCatalog.find((provider) => provider.id === entry.providerId)?.label || 'Connection',
          ...
          config: sanitizeConnectionConfig(entry.providerId, entry.config || {}, defaultManifestPath),
          capabilities: entry.capabilities || providerFactories[entry.providerId]?.getCapabilities?.() || {},
