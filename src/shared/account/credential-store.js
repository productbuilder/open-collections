...
function githubStableAccount(config = {}, options = {}) {
	...
	const path = options.legacyPath
		? normalizeGithubPathLegacy(config.path)
		: normalizeGithubPath(config.path);
	if (!owner || !repo) {
		return "";
	}
...
function credentialAccountsFor(source = {}, config = {}) {
	...
	if (source?.providerId === "github") {
		const stableCanonical = githubStableAccount(config, {
			legacyPath: false,
		});
		if (stableCanonical) {
...
export function createCredentialStore() {
	...
	return {
		async storeSourceSecret(source, config = {}) {
			...
			const hasSecret = Object.values(payload).some((value) =>
				String(value || "").trim(),
			);
			if (!hasSecret) {
				for (const account of accounts) {
					try {
						...
						logCredential("store:delete-empty-secret:ok", {
							namespace,
							account,
							summary,
						});
		...
		async loadSourceSecret(source, config = {}) {
			...
			for (const account of accounts) {
				try {
					stored = await platform.getCredential({
						namespace,
						account,
					});
			...
			if (source?.providerId === "github") {
				...
				return mergeSecretPayload(source, config, {
					token: decodedToken,
				});
			}
			...
			try {
				...
				logCredential("load:hit:s3", {
					...
					hasAccessKey: Boolean(
						String(parsed?.accessKey || "").trim(),
					),
					hasSecretKey: Boolean(
						String(parsed?.secretKey || "").trim(),
					),
				});
		...
		async deleteSourceSecret(source) {
			...
			const accounts = credentialAccountsFor(
				source,
				source?.config || {},
			);
