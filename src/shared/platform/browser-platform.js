...
async function pickFilesWithPicker({
	types,
	multiple = false,
	accept = "",
} = {}) {
...
export const browserPlatform = createPlatformApi({
	...
	async saveTextFile(
		...
		if (typeof window.showSaveFilePicker === "function") {
			...
			return {
				handle: nextHandle,
				path: "",
				name: nextHandle.name || suggestedName,
			};
		}
	...
	async readTextFile(pathOrHandle) {
		...
		throw new Error(
			"Browser platform requires a file handle to read text.",
		);
	},
	...
	async writeTextFile(pathOrHandle, text) {
		...
		throw new Error(
			"Browser platform requires a file handle to write text.",
		);
	},
