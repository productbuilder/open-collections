...
function unsupportedFeatureError(feature, details = "") {
	...
	return new Error(
		`${feature} is not supported on the Capacitor adapter.${suffix}`,
	);
}
...
async function openFileWithInputFallback(
	accept,
	suggestedType = "application/octet-stream",
) {
...
async function pickFilesWithCapacitor({
	extensions = [],
	multiple = false,
} = {}) {
	const { FilePicker } = getCapacitorPlugins();
	...
	const files = Array.isArray(result?.files)
		? result.files.map(normalizeCapacitorPickedFile).filter(Boolean)
		: [];
	return files;
}
...
export const capacitorPlatform = createPlatformApi({
	...
	async saveJsonFile(data, options = {}) {
		...
		await triggerDownload(
			text,
			options.suggestedName || "data.json",
			"application/json",
		);
		return {
			handle: null,
			path: "",
			name: options.suggestedName || "data.json",
		};
	},
	...
	async openDirectory() {
		...
		throw unsupportedFeatureError(
			"Directory selection",
			"Use cloud providers (GitHub/S3) on mobile.",
		);
	},
	...
	async readTextFile(pathOrHandle) {
		...
		throw unsupportedFeatureError(
			...
			"Provide a File object returned by openTextFile/openJsonFile.",
		);
	},
	...
	async writeTextFile(pathOrHandle, text) {
		...
		throw unsupportedFeatureError(
			"In-place file writes",
			"Use saveTextFile() to export content on mobile.",
		);
	},
	...
	async setCredential({ namespace, account, secret }) {
		sessionCredentials.set(
			credentialKey(namespace, account),
			String(secret ?? ""),
		);
	},
