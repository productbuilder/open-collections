...
class TauriFileHandle {
	...
	async getFile() {
		const bytes = await invoke("platform_read_binary_file", {
			path: this.path,
		});
	...
	async createWritable() {
		...
		return {
			async write(value) {
				...
				await invoke("platform_write_text_file", {
					path,
					text: String(value ?? ""),
				});
			},
...
class TauriDirectoryHandle {
	...
	async getDirectoryHandle(name, options = {}) {
		const nextPath = await invoke("platform_join_path", {
			base: this.path,
			name,
		});
		const exists = await invoke("platform_directory_exists", {
			path: nextPath,
		});
	...
	async getFileHandle(name, options = {}) {
		...
		if (!exists && options?.create) {
			await invoke("platform_write_binary_file", {
				path: nextPath,
				bytes: [],
			});
	...
	async *entries() {
		const entries = await invoke("platform_read_directory", {
			path: this.path,
		});
...
async function filesFromPaths(paths = []) {
	const files = await Promise.all(
		(Array.isArray(paths) ? paths : []).map(async (path) => {
			try {
...
async function subscribeWithWebviewApi(handler) {
	...
	return webview.onDragDropEvent(async (event) => {
		const payload = normalizeDropPayload(
			event?.payload?.type,
			event?.payload || {},
		);
...
export const tauriPlatform = createPlatformApi({
	...
	async saveTextFile(
		text,
		{ suggestedName = "file.txt", handle = null } = {},
	) {
		if (handle?.path) {
			await invoke("platform_write_text_file", {
				path: handle.path,
		...
		const result = await invoke("platform_save_text_file", {
			text: String(text ?? ""),
			suggestedName,
		});
		if (!result) {
	...
	async saveJsonFile(data, options = {}) {
		...
		return this.saveTextFile(text, {
			suggestedName: options.suggestedName || "data.json",
	...
	async setCredential({ namespace, account, secret }) {
		...
		try {
			await invoke("platform_set_credential", {
				namespace,
				account,
	...
	async getCredential({ namespace, account }) {
		...
		try {
			const value = await invoke("platform_get_credential", {
				namespace,
				account,
			});
