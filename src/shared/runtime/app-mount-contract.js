...
export function createAppRuntimeContext({
  ...
  if (!(target instanceof Element)) {
    throw new Error('createAppRuntimeContext requires a mount target element.');
  }
...
export function createWebComponentAppAdapter({
  ...
  if (!appId || !tagName) {
    throw new Error('createWebComponentAppAdapter requires appId and tagName.');
  }
  ...
  function applyRuntimeAttributes(targetElement, context, config = {}) {
    ...
    const mapped = typeof mapConfigToAttributes === 'function'
    ...
    for (const [key, value] of Object.entries(mapped)) {
      ...
      targetElement.setAttribute(key, value === true ? '' : String(value));
    }
  }
  ...
  return {
    mount(context) {
      ...
      context.emit(APP_LIFECYCLE_EVENTS.MOUNTED, { config: context.config });
      return {
        update(nextConfig = {}) {
          const mergedConfig = {
            ...context.config,
          ...
          context.emit(APP_LIFECYCLE_EVENTS.UPDATED, { config: mergedConfig });
        },
        unmount() {
          if (!element) {
            return;
