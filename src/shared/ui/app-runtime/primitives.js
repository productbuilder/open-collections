...
export function renderLoaderState({ title = 'Loading…', description = 'Please wait while we load this section.' } = {}) {
  return renderRuntimeState({ title, description, tone: 'loading' });
}

export function renderEmptyState({ title = 'No content yet', description = 'There is nothing to show in this section yet.' } = {}) {
  return renderRuntimeState({ title, description, tone: 'neutral' });
}

export function renderErrorState({ title = 'Something went wrong', description = 'Please retry or switch to standalone mode for diagnostics.' } = {}) {
  return renderRuntimeState({ title, description, tone: 'error' });
}

export function renderSuccessState({ title = 'Done', description = 'Your changes were applied successfully.' } = {}) {
